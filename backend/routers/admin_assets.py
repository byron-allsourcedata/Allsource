...
@router.get("/")
def partners_assets(
    get_partners_assets_service: PartnersAssetService = Depends(
        get_partners_assets_service
    ),
):
    ...
    if not assets.get("status"):
        ...
        raise HTTPException(
            ...
            detail=error.get("message", "Unknown error occurred"),
        )
...
async def create_asset(
    ...
    created_asset = await get_partners_assets_service.create_asset(
...
async def update_asset(
    ...
    updated_asset = await get_partners_assets_service.update_asset(
        asset_id, description, type, file
    )
