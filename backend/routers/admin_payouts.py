...
def update_payouts_partner(
    payouts_partner_request: PayoutsPartnerRequest,
    referral_service: PayoutsService = Depends(get_payouts_service),
    referral_payout_id: int = Path(..., description="referral payout id"),
):
    return referral_service.update_payouts_partner(
        referral_payout_id,
        payouts_partner_request.text,
        payouts_partner_request.confirmation_status,
    )


@router.get("/pay-out-referrals/{partner_id}")
def pay_out_referrals(
    referral_service: PayoutsService = Depends(get_payouts_service),
...
def pay_overview_payout_history(
    referral_service: PayoutsService = Depends(get_payouts_service),
    ...
    return referral_service.pay_overview_payout_history(
        page=page, per_page=per_page, from_date=from_date, to_date=to_date
    )
