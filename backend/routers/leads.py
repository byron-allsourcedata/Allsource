...
async def get_leads(
        ...
        time_spent: str = Query(None, description="time spent on the page in minutes"),
...
async def download_leads(
            ...
            to_time: str = Query(None, description="End time in integer format"),
            leads_service: LeadsService = Depends(get_leads_service)):
    result = leads_service.download_leads(from_date=from_date, to_date=to_date, regions=regions, page_visits=page_visits, average_time_spent=average_time_spent,
                                          behavior_type=behavior_type, status=status, recurring_visits=recurring_visits,sort_by=sort_by,
                                          sort_order=sort_order, search_query=search_query, from_time=from_time, to_time=to_time)
    if result:
        return StreamingResponse(result, media_type="text/csv",
                                 headers={"Content-Disposition": "attachment; filename=data.csv"})
    return BaseEnum.FAILURE
