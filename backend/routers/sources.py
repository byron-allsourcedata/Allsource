...
def get_sources(
    ...
    created_date_start: Optional[datetime] = Query(
        None, description="Start date of creation interval"
    ),
    ...
    return {"source_list": source_list, "count": count}
...
def get_sample_customers_list(
    sources_service: AudienceSourceService, source_type: str = Query(...)
):
    ...
    return FileResponse(
        ...
        media_type="text/csv",
        headers={
            "Content-Disposition": "attachment; filename=sample-customers-list.csv"
        },
    )
