...
def get_sample_suppression_list(suppression_service: SuppressionService = Depends(get_suppression_service),
                                user: User = Depends(check_user_authorization)):
    file_path = suppression_service.get_sample_suppression_list()
    return FileResponse(file_path, media_type="text/csv",
                        headers={"Content-Disposition": "attachment; filename=sample-suppression-list.csv"})
...
async def process_suppression_multiple_emails(suppression_request: SuppressionRequest,
                                              suppression_service: SuppressionService = Depends(
