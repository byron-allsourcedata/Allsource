...
def get_account_details(settings_service: SettingsService = Depends(get_settings_service),
...
def change_account_details(account_details: AccountDetailsRequest,
...
def change_email_account_details(settings_service: SettingsService = Depends(get_settings_service),
                                 token: str = Query(...), mail: str = Query(...)):
...
@router.get("/teams")
def get_teams(settings_service: SettingsService = Depends(get_settings_service),
              user: dict = Depends(check_user_authorization_without_pixel)):
    return settings_service.get_team_members(user=user)


@router.get("/teams/pending-invations")
def get_pending_invations(settings_service: SettingsService = Depends(get_settings_service),
                          user: dict = Depends(check_user_authorization_without_pixel)):
    return settings_service.get_pending_invations(user=user)


@router.put("/teams")
def change_teams(teams_details: TeamsDetailsRequest, settings_service: SettingsService = Depends(get_settings_service),
                 user: dict = Depends(check_user_authorization_without_pixel)):
...
@router.post("/teams")
def invite_user(teams_details: TeamsDetailsRequest, settings_service: SettingsService = Depends(get_settings_service),
                user: dict = Depends(check_user_authorization_without_pixel)):
    if user.get('team_member'):
        team_member = user.get('team_member')
        if team_member.get('team_access_level') not in {TeamAccessLevel.ADMIN.value, TeamAccessLevel.OWNER.value}:
    ...
    return settings_service.invite_user(user=user, invite_user=teams_details.invite_user,
                                        access_level=teams_details.access_level)


@router.post("/teams/change-user-role")
def change_user_role(teams_details: TeamsDetailsRequest,
                     settings_service: SettingsService = Depends(get_settings_service),
                     user: dict = Depends(check_user_authorization_without_pixel)):
    if user.get('team_member'):
        team_member = user.get('team_member')
    ...
    return settings_service.change_user_role(user=user, email=teams_details.invite_user,
                                             access_level=teams_details.access_level)


@router.get("/teams/check-team-invitations-limit")
def check_team_invitations_limit(settings_service: SettingsService = Depends(get_settings_service),
                                 user: dict = Depends(check_user_authorization_without_pixel)):
    if user.get('team_member'):
        team_member = user.get('team_member')
        if team_member.get('team_access_level') not in {TeamAccessLevel.ADMIN.value, TeamAccessLevel.OWNER.value}:
...
def switch_overage(settings_service: SettingsService = Depends(get_settings_service),
    ...
    return settings_service.switch_overage(user=user)
...
def delete_card(payment_card: PaymentCard, settings_service: SettingsService = Depends(get_settings_service),
    ...
    if user.get('team_member'):
        ...
        if team_member.get('team_access_level') != TeamAccessLevel.ADMIN.value or team_member.get(
                'team_access_level') != TeamAccessLevel.OWNER.value:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Access denied. Admins only."
            )
...
def get_api_details(settings_service: SettingsService = Depends(get_settings_service),
                    user: dict = Depends(check_user_authorization_without_pixel)):
    return settings_service.get_api_details(user=user)


@router.put("/api-details")
def change_api_details(api_keys_request: ApiKeysRequest,
                       settings_service: SettingsService = Depends(get_settings_service),
                       user: dict = Depends(check_user_authorization_without_pixel)):
