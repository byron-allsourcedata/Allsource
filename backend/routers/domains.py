...
def create_domain(
    ...
    if not domain:
        raise HTTPException(
            status_code=400, detail={"status": "Domain creation failed"}
        )
    return domain
...
def update_domain_name(
    ...
    return domain_service.update_domain_name(
        domain_id=domain.id, domain_name=request.domain_name
    )
