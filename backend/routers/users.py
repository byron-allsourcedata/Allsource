...
async def get_notification(
    request: Optional[DismissNotificationsRequest] = None,
    notification_service: Notification = Depends(get_notification_service),
    user=Depends(check_user_authentication),
):
...
async def login_user(
    ...
    return UserLoginFormResponse(
        ...
        shopify_status=user_data.get("shopify_status"),
    )
...
async def verify_token(
    user: UsersAuth = Depends(get_users_auth_service), token: str = Query(...)
):
...
async def reset_password(
    ...
    user: UsersAuth = Depends(get_users_auth_service),
):
...
async def update_password(
    update_data: UpdatePassword, user: UsersService = Depends(get_users_service)
):
