...
async def process_rmq_message(
    ...
    try:
        ...
        for record in batch:
            ...
            for phone_field in ["phone_mobile1", "phone_mobile2"]:
                ...
                if not existing_verification:
                    is_verify = next(
                        (
                            verification.is_verify
                            for verification in verifications
                            if verification.phone == phone_number
                        ),
                        False,
                    )
        ...
        if len(verifications):
            verification_data = [
                {"phone": v.phone, "status": v.status, "is_verify": v.is_verify}
                for v in verifications
            ]

            insert_stmt = (
                insert(AudiencePhoneVerification)
                ...
                for cat in validations.values():
                    for rule in cat:
                        if key in rule:
                            ...
                            rule[key]["count_cost"] = str(write_off_funds.quantize(Decimal("0.01")))
