...
def set_bing_ads_audience_contacts(
    ...
    try:
        ...
        authorization_data = AuthorizationData(
            developer_token=developer_token, authentication=authentication
        )
        ...
        response = campaign_service.SetAudienceContacts(
            AudienceId=audience_id, AudienceContacts=audience_contacts_array
        )
...
def create_campaign(
    ...
    try:
        response = campaign_service.AddCampaigns(
            AccountId=authorization_data.account_id, Campaigns=campaigns
        )
        if response and response.CampaignIds and response.CampaignIds["long"]:
            ...
            if (
                ...
                for error in response.PartialErrors["BatchError"]:
                    if error.Code == 1115:
                        raise Exception(
                            "A campaign with the same name already exists. Please choose a different name."
                        )
                    else:
                        raise Exception(f"Error {error.Code}: {error.Message}")
            else:
                raise Exception(
                    "Failed to create campaign. API response does not contain campaign ID."
                )
...
def add_customer_list_to_campaign_bulk(
    ...
    assoc = BulkCampaignCustomerListAssociation(
        biddable_campaign_criterion=bcc, campaign_name=None, audience_name=None
    )
    ...
    for bulk_entity in bulk_manager.upload_entities(upload_params):
        if isinstance(bulk_entity, BulkCampaignCustomerListAssociation):
            if hasattr(bulk_entity, "has_errors") and bulk_entity.has_errors:
                ...
                print(
                    f"Success: CampaignId={bulk_entity.biddable_campaign_criterion.CampaignId}, "
...
async def main():
    ...
    add_emails_to_audience(
        DEVELOPER_TOKEN,
        CLIENT_ID,
        CLIENT_SECRET,
        refresh_token,
        569722874,
        ["email@gmail.com", "email1@gmail.com", "email2@gmail.com"],
    )
