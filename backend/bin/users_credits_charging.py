...
async def get_account_notification_by_title(session, title: str) -> str:
    return (
        session.query(AccountNotification)
        .filter(AccountNotification.title == title)
        .first()
    )


async def save_account_notification(
...
async def on_message_received(message, session, subscription_service):
    try:
        ...
        if not user or not contact_credits:
            logging.error(
                "Invalid user or subscription plan",
                extra={"customer_id": customer_id, "plan_id": plan_id},
            )
            await message.ack()
            return
        ...
        if user.leads_credits > 0:
            ...
            if user.is_leads_auto_charging:
                ...
                if lead_user_count >= QUANTITY:
                    logging.info(
                        f"{user.full_name} user_id {user.id} charge {QUANTITY} leads by {contact_credits.price}"
                    )
                    last_payment_intent = get_last_payment_intent(customer_id)
                    if last_payment_intent:
                        users_inlocked_five_x_five_user = (
                            session.query(UsersUnlockedFiveXFiveUser)
                            .filter(
                                UsersUnlockedFiveXFiveUser.transaction_id
                                == last_payment_intent.id
                            )
                            .first()
                        )
                        if not users_inlocked_five_x_five_user:
                            logging.error(
                                f"users_inlocked_five_x_five_user is None, last_transaction_id {last_payment_intent.id}"
                            )
                            await message.reject(requeue=True)
                            return
                    result = purchase_product(
                        customer_id,
                        contact_credits.stripe_price_id,
                        QUANTITY,
                        "leads_credits",
                    )
                    if result["success"]:
                        ...
                        if (
                            not session.query(UsersUnlockedFiveXFiveUser)
