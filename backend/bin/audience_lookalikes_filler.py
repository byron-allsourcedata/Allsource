...
def get_enrichment_user_column_map() -> Dict[str, Any]:
    return {
        ...
        "zip_code5": coalesce(
            cast(EnrichmentPersonalProfiles.zip_code5, String), "00000"
        ).label("zip_code5"),
        # — Financial Records —
...
def build_dynamic_query_and_config(
    db_session: Session, sig: Dict[str, float]
) -> Tuple:
    column_map = {
        k: v
        for k, v in get_enrichment_user_column_map().items()
        if k
        not in {
            "job_title",
            "company_name",
    ...
    query = (
        db_session.query(*select_columns)
        ...
        .outerjoin(
            EnrichmentLifestyle, EnrichmentLifestyle.asid == EnrichmentUser.asid
        )
        .outerjoin(
    ...
    normalization_config = NormalizationConfig(
        numerical_features=[], ordered_features={}, unordered_features=unordered
    )
...
def fetch_user_profiles(
    db_session: Session, audience_lookalike: AudienceLookalikes
) -> List[Dict]:
...
def train_and_save_model(
    ...
    trained = similar_audience_service.get_trained_model(
    ...
    try:
        ...
        enrichment_lookalike_scores = (
            ...
            .filter(
                ...
                ~EnrichmentLookalikeScore.enrichment_user_id.in_(
                    source_uid_select
                ),
            )
        ...
        scores = [
            float(s.score)
            for s in enrichment_lookalike_scores
        ...
        persons = [
            ...
            for enrichment_lookalike_score in enrichment_lookalike_scores
        ]
