...
async def main():
    ...
    config = NormalizationConfig(
        numerical_features=["NumberOfChildren", "LengthOfResidenceYears"],
        unordered_features=[
            "PersonGender",
            "HasChildren",
            "HomeownerStatus",
    ...
    scores = SimilarAudiencesScoresService(
        db=db,
        models=models,
    ...
    query = db.query(
        ...
        EnrichmentUser.estimated_household_income_code.label(
            "EstimatedHouseholdIncomeCode"
        ),
        EnrichmentUser.estimated_current_home_value_code.label(
        ...
        EnrichmentUser.length_of_residence_years.label(
    ...
    scores.calculate_scores(
        lookalike_id, query, user_id_key="id", config=config
    )
