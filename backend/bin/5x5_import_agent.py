                    ...
                    with tempfile.NamedTemporaryFile(delete=True) as temp_file:
                        ...
                        for _, row in df.iterrows():
                            ...
                            job_title_last_updated = pd.to_datetime(row.get('JOB_TITLE_LAST_UPDATED', None), unit='s',
                                                                    errors='coerce')
                            five_x_five_hems = FiveXFiveUser(
                                ...
                                last_updated=None if pd.isna(last_updated) else last_updated,
                                personal_emails_last_seen=None if pd.isna(
                                    personal_emails_last_seen) else personal_emails_last_seen,
                                company_last_updated=None if pd.isna(company_last_updated) else company_last_updated,
                                job_title_last_updated=None if pd.isna(
                                    job_title_last_updated) else job_title_last_updated,
                            )
                            db_session.merge(five_x_five_hems)
                        db_session.commit()
