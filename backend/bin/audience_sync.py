...
async def fetch_consignments_data(
    consignments_url: str, access_token: str
) -> Optional[dict]:
    try:
...
async def bigcommerce_process(
    ...
    for order in orders:
        ...
        try:
            ...
            logging.error(
                f"Error fetching customer info for customer_id {customer_id}: {e}"
            )
            continue
        ...
        consignments_data = await fetch_consignments_data(
            consignments_url, access_token
        )
        ...
        for consignment in consignments_data.get("shipping", []):
            for item in consignment.get("line_items", []):
                ...
                try:
                    ...
                    logging.error(
                        f"Error fetching product info for product_id {product_id}: {e}"
                    )
                    continue
                ...
                if product_info["categories"]:
                    for category_id in product_info["categories"]:
                        categories_purchased[category_id] = (
                            categories_purchased.get(category_id, 0) + 1
                        )
            ...
            if order_date_str:
                try:
                    order_date = datetime.strptime(
                        order_date_str, "%a, %d %b %Y %H:%M:%S +0000"
                    )
    ...
    try:
        ...
        if data_source == "bigcommerce":
            user_integration = (
                ...
                .filter(
                    (UserIntegration.id == domain_id)
                    & (UserIntegration.service_name == data_source)
                )
                .first()
            )
            if user_integration:
                result_process = await bigcommerce_process(
                    user_integration.shop_domain,
