...
async def aud_smarts_matching(message: IncomingMessage, db_session: Session, connection):
    try:
        ...
        for five_x_five_user_id in five_x_five_users_ids:
            matched_person = AudienceSmartPerson(
        ...
        processed_records, total_records = db_session.execute(
            update(AudienceSmart)
            .where(AudienceSmart.id == aud_smart_id)
            .values(
                processed_active_segment_records=AudienceSmart.processed_active_segment_records + len(five_x_five_users_ids)
            )
            .returning(AudienceSmart.processed_active_segment_records, AudienceSmart.total_records)
        ).fetchone()
                    
        db_session.commit()
            
        await send_sse(connection, user_id, {"smart_audience_id": aud_smart_id, "total": total_records, "processed": processed_records})
        logging.info(f"ack")
        await message.ack()
