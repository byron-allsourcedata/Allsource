...
def check_blacklist_domain_email(
    business_email, personal_emails, additional_personal_emails, mail_domains
):
...
async def fetch_users_by_domain(db_session, company_domains, job_titles, mail_domain):
    ...
    for domain in company_domains:
        ...
        users = (
            db_session.query(FiveXFiveUser)
            .filter(FiveXFiveUser.company_domain == domain)
            .all()
        )
        ...
        for user in users:
            if user.job_title and any(
                word.lower() in job_titles for word in user.job_title.split()
            ):
                email = check_blacklist_domain_email(
                    user.business_email,
...
async def main():
    ...
    parser.add_argument(
        "input_file", help="Path to the input file with company domains"
    )
