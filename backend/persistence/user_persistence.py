...
class UserPersistence:
    ...
    def user_plan_info_db(self, user_id):
        return self.db.query(UserSubscriptionPlan, SubscriptionPlan).join(SubscriptionPlan, UserSubscriptionPlan.plan_id == SubscriptionPlan.id).filter(UserSubscriptionPlan.user_id == user_id).order_by(UserSubscriptionPlan.id.desc()).first()
