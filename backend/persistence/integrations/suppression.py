...
class IntegrationsSuppressionPersistence:
    ...
    def get_last_leads_suppression(
        self, domain_id: int, integration_id: int
    ) -> LeadsSupperssion:
        try:
            return (
                self.db.query(LeadsSupperssion)
                .filter(
                    LeadsSupperssion.domain_id == domain_id,
                    LeadsSupperssion.integrations_id == integration_id,
                )
                .order_by(LeadsSupperssion.id.decs)
                .limit(1)
                .first()
            )
        except Exception:
