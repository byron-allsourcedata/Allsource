...
class ServiceIntegrationsPersistence(IntegrationPersistenceABC):
    ...
    def save_customer(self, customer: dict, user_id: int) -> str:
        ...
        if existing_lead_user:
            integration_lead_id = getattr(
                existing_lead_user, self.integration_leads_column
            )
            if integration_lead_id:
                self.session.query(self.model).filter(
                    self.model.id == integration_lead_id
                ).update(customer)
                self.session.commit()
                return "MATCHED"
            else:
                ...
                self.session.query(LeadUser).filter(
                    LeadUser.id == existing_lead_user.id
                ).update(
                    {
            ...
            existing_record = (
                ...
                .filter(self.model.email == customer["email"])
                .first()
            )
            if existing_record:
