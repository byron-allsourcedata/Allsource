...
class ReferralPayoutsPersistence:
    ...
    def update_payouts_partner_confirmation_status(
        self, referral_payout_id: int, confirmation_status, text=None
    ):
        referral_payout = (
            self.db.query(ReferralPayouts)
            .filter(ReferralPayouts.id == referral_payout_id)
            .first()
        )
    ...
    def get_referral_payouts_by_parent_id(self, parent_id):
        return (
            self.db.query(ReferralPayouts)
            .filter(ReferralPayouts.parent_id == parent_id)
            .all()
        )
    ...
    def get_referral_payouts_by_parent_ids(self, user_ids):
        return (
            self.db.query(ReferralPayouts)
            .filter(ReferralPayouts.parent_id.in_(user_ids))
            .all()
        )

    def get_total_payouts_to_refferal(
        self, partner_id, reward_type, year=None, month=None
    ):
        query = (
            self.db.query(ReferralPayouts)
    ...
    def get_referral_payouts_by_partner_id(
        self,
        year,
        month,
        partner_id,
