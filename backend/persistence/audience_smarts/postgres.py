...
class AudienceSmartsPostgresPersistence(AudienceSmartsPersistenceInterface):
    ...
    def get_persons_by_smart_aud_id(self, smart_audience_id, sent_contacts, fields) -> List[PersonRecord]:
        contact_fields = [
        ...
        col_names = [col.key for col in contact_fields] + [col.key for col in profile_fields]
    ...
    def get_synced_persons_by_smart_aud_id(self, data_sync_id, enrichment_field_names) -> List[SyncedPersonRecord]:
        enrichment_fields = [
    ...
    def get_synced_person_ids(
            self, data_sync_id: int
    ) -> List[UUID]:
        return [
            row[0]
            for row in (
