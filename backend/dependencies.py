...
def get_referral_user_persistence(
    db: Session = Depends(get_db),
...
def get_user_domain_persistence(
    db: Session = Depends(get_db),
...
def get_accounts_service(
    referral_user_persistence: ReferralUserPersistence = Depends(
        get_referral_user_persistence
    ),
...
def get_referral_service(
    ...
    referral_payouts_persistence: ReferralPayoutsPersistence = Depends(
        get_referral_payouts_persistence
    ),
    referral_user_persistence: ReferralUserPersistence = Depends(
        get_referral_user_persistence
    ),
    ...
    return ReferralService(
        referral_persistence_discount_code_service=referral_persistence_discount_code,
        user_persistence=user_persistence,
...
def check_user_authentication(
    ...
    if not user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail={"status": "NOT_FOUND"}
        )
...
def check_user_authorization(
    ...
    if auth_status != UserAuthorizationStatus.SUCCESS:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail={"status": auth_status.value}
        )
    return user
...
def check_user_partner(
    ...
    if not user["is_partner"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail={"status": "Aссess Forbidden"}
        )
    return user
...
def get_domain_service(
    user_domain_persistence: UserDomainsPersistence = Depends(
...
def get_plans_service(
    ...
    return PlansService(
        plans_persistence=plans_persistence, subscription_service=subscription_service
    )
...
def check_user_admin(
    ...
    if "admin" not in user["role"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail={"status": "FORBIDDEN"}
        )
    return user
...
def check_api_key(
    ...
    if maximiz_api_key:
        ...
        if domains:
            return domains[0]
        raise HTTPException(
            status_code=404, detail={"status": DomainStatus.DOMAIN_NOT_FOUND.value}
        )
    raise HTTPException(
        status_code=401,
