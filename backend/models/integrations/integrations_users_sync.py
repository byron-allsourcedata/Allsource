...
class IntegrationUserSync(Base):
    ...
    sync_type = Column(
        data_sync_type, nullable=False, server_default=text("'pixel'::data_sync_type")
    )
    ...
    no_of_contacts = Column(Integer, nullable=True, server_default=text("0"))
    created_by = Column(String(256), nullable=True)
    last_lead_sync_id = Column(BigInteger, ForeignKey("leads_users.id"), nullable=True)
    hook_url = Column(VARCHAR, nullable=True)
    last_sent_lead_id = Column(BigInteger, ForeignKey("leads_users.id"), nullable=True)
