from sqlalchemy import Column, Integer, TEXT, VARCHAR, TIMESTAMP

from .base import Base


class FiveXFiveUser(Base):
    __tablename__ = '5x5_users'

    id = Column(Integer, primary_key=True)
    up_id = Column(VARCHAR(64), nullable=True)
    cc_id = Column(VARCHAR(64), nullable=True)
    first_name = Column(VARCHAR(64), nullable=True)
    programmatic_business_emails = Column(TEXT, nullable=True)
    mobile_phone = Column(TEXT, nullable=True)
    direct_number = Column(TEXT, nullable=True)
    gender = Column(VARCHAR(4), nullable=True)
    personal_phone = Column(TEXT, nullable=True)
    business_email = Column(VARCHAR(64), nullable=True)
    personal_emails = Column(VARCHAR(64), nullable=True)
    last_name = Column(VARCHAR(64), nullable=True)
    personal_city = Column(VARCHAR(64), nullable=True)
    personal_state = Column(VARCHAR(8), nullable=True)
    company_name = Column(VARCHAR(32), nullable=True)
    company_domain = Column(VARCHAR(32), nullable=True)
    company_phone = Column(VARCHAR(128), nullable=True)
    company_sic = Column(VARCHAR(64), nullable=True)
    company_address = Column(VARCHAR(64), nullable=True)
    company_city = Column(VARCHAR(32), nullable=True)
    company_state = Column(VARCHAR(32), nullable=True)
    company_zip = Column(VARCHAR(8), nullable=True)
    company_linkedin_url = Column(VARCHAR(64), nullable=True)
    company_revenue = Column(VARCHAR(64), nullable=True)
    company_employee_count = Column(VARCHAR(64), nullable=True)
    net_worth = Column(VARCHAR(64), nullable=True)
    job_title = Column(VARCHAR(64), nullable=True)
    last_updated = Column(TIMESTAMP, nullable=True)
    personal_emails_last_seen = Column(TIMESTAMP, nullable=True)
    company_last_updated = Column(TIMESTAMP, nullable=True)
    job_title_last_updated = Column(TIMESTAMP, nullable=True)
    first_name_id = Column(Integer, nullable=True)
    last_name_id = Column(Integer, nullable=True)
    age_min = Column(Integer, nullable=True)
    age_max = Column(Integer, nullable=True)
    additional_personal_emails = Column(TEXT, nullable=True)
    linkedin_url = Column(VARCHAR(128), nullable=True)
    personal_address = Column(VARCHAR(64), nullable=True)
    personal_address_2 = Column(VARCHAR(64), nullable=True)
    personal_zip = Column(VARCHAR(16), nullable=True)
    personal_zip4 = Column(VARCHAR(16), nullable=True)
    professional_zip = Column(VARCHAR(8), nullable=True)
    married = Column(VARCHAR(4), nullable=True)
    children = Column(VARCHAR(4), nullable=True)
    income_range = Column(VARCHAR(64), nullable=True)
    homeowner = Column(VARCHAR(4), nullable=True)
    seniority_level = Column(VARCHAR(64), nullable=True)
    department = Column(VARCHAR(64), nullable=True)
    professional_address = Column(VARCHAR(64), nullable=True)
    professional_address_2 = Column(VARCHAR(32), nullable=True)
    professional_city = Column(VARCHAR(32), nullable=True)
    professional_state = Column(VARCHAR(4), nullable=True)
    professional_zip4 = Column(VARCHAR(4), nullable=True)
    primary_industry = Column(VARCHAR(128), nullable=True)
    business_email_validation_status = Column(VARCHAR(64), nullable=True)
    business_email_last_seen = Column(TIMESTAMP, nullable=True)
    personal_emails_validation_status = Column(VARCHAR(64), nullable=True)
    work_history = Column(TEXT, nullable=True)
    education_history = Column(TEXT, nullable=True)
    company_description = Column(TEXT, nullable=True)
    related_domains = Column(VARCHAR(128), nullable=True)
    social_connections = Column(VARCHAR(32), nullable=True)
    dpv_code = Column(VARCHAR(2), nullable=True)
