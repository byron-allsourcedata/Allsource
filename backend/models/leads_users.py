...
class LeadUser(Base):
    ...
    five_x_five_user_id = Column(
        BigInteger, ForeignKey("5x5_users.id", ondelete="CASCADE"), nullable=True
    )
    ...
    first_visit_id = Column(
        BigInteger, ForeignKey("leads_visits.id", ondelete="CASCADE"), nullable=True
    )
    is_converted_sales = Column(Boolean, nullable=True, server_default=text("false"))
    is_returning_visitor = Column(Boolean, nullable=True, server_default=text("false"))
    domain_id = Column(
    ...
    company_id = Column(
        BigInteger, ForeignKey("leads_companies.id", ondelete="CASCADE"), nullable=True
    )
