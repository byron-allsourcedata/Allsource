...
class SubscriptionService:
            ...
            if subscription_data.stripe_request_created_at is not None:
                if isinstance(subscription_data.stripe_request_created_at, str):
                    subscription_datetime = datetime.strptime(
                        subscription_data.stripe_request_created_at, "%Y-%m-%d %H:%M:%S"
                    )
                else:
    ...
    def get_user_subscription_with_trial_status(self, user_id):
        ...
        if result:
            user_subscription, is_free_trial, trail_days, currency, price, alias = (
                result
            )
    ...
    def create_subscription_transaction(self, user_id, stripe_payload: dict):
        ...
        start_date = datetime.fromtimestamp(start_date_timestamp, timezone.utc).replace(
            tzinfo=None
        )
        end_date = datetime.fromtimestamp(end_date_timestamp, timezone.utc).replace(
            tzinfo=None
        )
        created_at = get_utc_aware_date_for_postgres()
        currency = stripe_payload.get("data").get("object").get("currency")
        amount = Decimal(
            stripe_payload.get("data").get("object").get("plan").get("amount_decimal")
        ) / Decimal(100)
    ...
    def update_team_members(self, team_owner_id, members_limit):
        if team_owner_id and members_limit != self.UNLIMITED:
            users = (
                self.db.query(Users).filter(Users.team_owner_id == team_owner_id).all()
            )
            if users:
                sorted_users = sorted(users, key=lambda x: x.added_on or datetime.min)
        ...
        if status == "active":
            ...
            if referral_parent_id and not payout_id:
                ...
                if partner and partner.commission and partner.is_active:
                    ...
                    reward_amount = round(
                        Decimal(partner.commission) / Decimal(100) * final_amount, 2
                    )
                    ...
                    if partner.master_id:
                        ...
                        if (
                            ...
                            master_reward_amount = round(
                                Decimal(master_partner.commission - partner.commission)
                                / Decimal(100)
                                * reward_amount,
                                2,
                            )
            ...
            if user_subscription and user_subscription.status == "active":
                if canceled_at:
                    user_subscription.cancel_scheduled_at = datetime.fromtimestamp(
                        canceled_at, timezone.utc
                    ).replace(tzinfo=None)
                if start_date > user_subscription.plan_start:
                    user_subscription.plan_start = start_date
                    user_subscription.updated_at = datetime.now(timezone.utc).replace(
                        tzinfo=None
                    )
                ...
                user.leads_credits = (
                    ...
                    else leads_credits - user.leads_credits
                )
                user.prospect_credits = prospect_credits
                user.current_subscription_id = user_subscription.id
                user.is_leads_auto_charging = True
    ...
    def get_invitation_limit(self, user_id):
        return (
            len(self.user_persistence_service.get_combined_team_info(user_id=user_id))
            + 1
        )
