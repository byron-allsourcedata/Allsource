...
class AudienceDataNormalizationService:
    ...
    def normalize(
        ...
        return self.normalize_dataframe(
            df=df, config=default_normalization_config()
        )

    def normalize_dataframe(
        self, df: DataFrame, config: NormalizationConfig
    ) -> Tuple[DataFrame, DataFrame]:
    ...
    def convert_ordered_features(
        self, df: DataFrame, rules: OrderedFeatureRules
    ):
    ...
    def fill_unknowns(self, df: DataFrame, cat_columns: List[str]):
        for cat in cat_columns:
            df.loc[:, cat] = (
                df[cat].fillna("U").astype(str).infer_objects(copy=False)
            )
        ...
        if "length_of_residence_years" in df.columns:
            df["length_of_residence_years"] = df[
                "length_of_residence_years"
            ].fillna(0)
    ...
    def merge_with_geo(self, df: DataFrame) -> DataFrame:
        ...
        df_with_geo = df.merge(
            df_geo, how="left", left_on="zip_code5", right_on="zip"
        )
        df_with_geo["state_city"] = (
            df_with_geo["state_name"] + "|" + df_with_geo["city"]
        )

        return df_with_geo

    def filter_columns(
        self, df_with_geo: DataFrame, config: NormalizationConfig
    ):
