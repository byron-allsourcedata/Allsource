...
class SimilarAudiencesScoresService:
    ...
    def get_lookalike_scores(
        self, source_uuid: UUID, lookalike_id: UUID, total_rows: int
    ) -> List[Tuple[UUID, Decimal]]:
        return self.enrichment_lookalike_scores_persistence.get_lookalike_scores(
            source_uuid, lookalike_id, total_rows
        )
    ...
    def get_config(self, significant_fields: dict):
        ...
        return NormalizationConfig(
            ordered_features={}, numerical_features=[], unordered_features=column_names
        )

    def calculate_score_dict_batch(
        self, model: CatBoostRegressor, persons: List[dict], config: NormalizationConfig
    ) -> List[float]:
        df = DataFrame(persons)
        return self.calculate_score_batches(model, df, config=config)

    def calculate_score_batches(
        self, model: CatBoostRegressor, df: DataFrame, config: NormalizationConfig
    ) -> List[float]:
