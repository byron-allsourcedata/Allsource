...
from services.similar_audiences.audience_profile_fetcher.interface import (
    ProfileFetcherInterface,
)
...
class ClickhouseProfileFetcher(ProfileFetcherInterface):
    ...
    def fetch_profiles(
        self, selected_columns: List[str], asids: List[UUID]
    ) -> List[dict]:
        ...
        result = [
            {
                **row,
                "zip_code5": pad_with_zeros(row["zip_code5"])
                if "zip_code5" in row
                else "00000",
            }
            for row in result
    ...
    def fetch_profiles_from_lookalike(
    ...
    def get_value_and_user_asids(
        self, db: Session, source_id: UUID
