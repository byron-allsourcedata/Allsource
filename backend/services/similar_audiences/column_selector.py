...
class AudienceColumnSelectorBase:
    ...
    def clickhouse_columns(self, significant_fields: dict) -> List[str]:
        return self.column_names(significant_fields)

    def cols(self, audience_lookalike: AudienceLookalikes):
        ...
        select_cols = [column_map[name] for name in selected_column_names] + [
            column_map["customer_value"]
        ]

        return select_cols

    def column_names(self, significant_fields: dict) -> List[str]:
        column_map = {
            "customer_value": (
                AudienceSourcesMatchedPerson.value_score.label("customer_value")
            ),
