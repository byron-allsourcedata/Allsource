...
class ShopifyIntegrationService:
    ...
    def get_shopify_token(self, shopify_data: ShopifyPayloadModel):
        try:
            shopify.Session.setup(
                api_key=ShopifyConfig.key, secret=ShopifyConfig.secret
            )
    ...
    def get_charge_by_id(self, user_data, charge_id):
        with shopify.Session.temp(
            user_data.shop_domain, ShopifyConfig.api_version, user_data.shopify_token
        ):
            charge = shopify.RecurringApplicationCharge.find(charge_id)
            return charge

    def get_shopify_shop_id(
        self, shopify_data: ShopifyPayloadModel, shopify_access_token: str
    ):
        shop_id = None
        with shopify.Session.temp(
            shopify_data.shop, ShopifyConfig.api_version, shopify_access_token
        ):
            shop = shopify.Shop.current()
            shop_id = shop.id
        return shop_id

    def create_new_recurring_charge(
        self, shopify_domain, access_token, plan, test_mode
    ):
        with shopify.Session.temp(
            shopify_domain, ShopifyConfig.api_version, access_token
        ):
            ...
            if response.status_code != 200:
                if response.status_code == 401:
                    return {
                        "status": "INCOMPLETE",
                        "message": "Authentication failed: Invalid API key or access token",
                    }
                else:
                    raise ValueError(
    ...
    def initialize_subscription_charge(self, plan: SubscriptionPlan, user: dict):
        ...
        if user.get("shopify_token") is None:
            return {
                "status": "INCOMPLETE",
                "message": "Shopify token not found, please install Shopify app",
            }

        return self.create_new_recurring_charge(
            shopify_domain=user.get("shop_domain"),
            access_token=user.get("shopify_token"),
            plan=plan,
            test_mode=test_mode,
        )
    ...
    def cancel_current_subscription(self, user: User):
        ...
        with shopify.Session.temp(
            user.get("shop_domain"),
            ShopifyConfig.api_version,
            user.get("shopify_token"),
        ):
    ...
    def create_webhooks_for_store(
        self, shopify_data: ShopifyPayloadModel, shopify_access_token
    ):
        with shopify.Session.temp(
            shopify_data.shop, ShopifyConfig.api_version, shopify_access_token
        ):
            shopify.Webhook.create(
                {
                    ...
                    "address": os.getenv("API_SITE_HOST_URL")
                    + "/api/subscriptions/shopify/billing/webhook",
                    "format": "json",
                }
            )
            shopify.Webhook.create(
    ...
    def __handle_request(
        ...
        if response.status_code not in {200, 201}:
            raise HTTPException(
                status_code=response.status_code, detail={"status": "error"}
            )
    ...
    def initialize_pixel(self, access_token, domain, user_id):
        ...
        """
        with shopify.Session.temp(
            domain.domain, ShopifyConfig.api_version, access_token
        ):
    ...
    def __set_pixel(self, user_id, domain, credentials):
        ...
        script_event_data = {"script_tag": {"event": "onload", "src": script_event_url}}
    ...
    def get_shopify_install_url(self, shop, r):
        ...
        url = session.create_permission_url(
            ShopifyConfig.scopes, ShopifyConfig.callback_uri, state
        )
        return url

    def handle_uninstalled_app(self, payload):
        shop_id = payload["id"]
        user_integration = self.integration_persistence.get_integration_by_shop_id(
            shop_id=shop_id
        )
        if user_integration:
            ...
            user_domains = (
                self.db.query(UserDomains)
                .filter(UserDomains.user_id == user_domain.user_id)
                .all()
            )
            if len(user_domains) > 1:
                ...
                self.db.query(UserDomains).filter(
                    ...
                    {UserDomains.is_pixel_installed: False}, synchronize_session=False
                )
    ...
    def verify_shopify_hmac(self, data: bytes, hmac_header: str) -> bool:
        digest = hmac.new(
            ShopifyConfig.secret.encode("utf-8"), data, hashlib.sha256
        ).digest()
    ...
    def oauth_shopify_redact(self, request_body, shopify_hmac_header):
        ...
        shop_redact_form: ShopifyShopRedactForm = (
            ShopifyShopRedactForm.model_validate_json(request_body.decode("utf-8"))
        )

        user_integration = self.integration_persistence.get_integration_by_shop_id(
            shop_id=shop_redact_form.shop_id
        )
        if user_integration:
            user = (
                self.db.query(User)
                .filter(User.shop_id == str(shop_redact_form.shop_id))
                .first()
            )
            user.shop_id = None
            ...
            user_subscription = (
                ...
                .filter(UserSubscriptions.id == str(shop_redact_form.shop_id))
                .first()
            )
            if user_subscription:
                user_subscription.cancel_scheduled_at = datetime.now(
                    timezone.utc
                ).replace(tzinfo=None)
    ...
    def oauth_shopify_callback(self, shop, r):
        ...
        user_integration = self.integration_persistence.get_integration_by_shop_url(
            shop_url=shop
        )
        ...
        self.__save_integration(
            ...
            domain_id=user_integration.domain_id,
        )
    ...
    def __save_integration(
        ...
        if not integration:
            raise HTTPException(
                ...
                detail={
                    "status": "error",
                    "detail": {"message": "Save integration failed"},
                },
            )
    ...
    def __create_or_update_shopify_customer(
        ...
        headers = {"X-Shopify-Token": access_token, "Content-Type": "application/json"}
        response = self.__handle_request(
            "POST", url, headers=headers, json=customer_json
        )
        return response.json().get("customers")

    def add_integration(
        self, credentials: IntegrationCredentials, domain, user: dict, shop_id=None
    ):
        if not credentials.shopify.shop_domain.startswith("https://"):
            credentials.shopify.shop_domain = (
                f"https://{credentials.shopify.shop_domain}"
            )
        shop_domain = (
            credentials.shopify.shop_domain.lower().lstrip("http://").lstrip("https://")
        )
        ...
        return {
            "status": "Successfully",
            "detail": {"service_name": SourcePlatformEnum.SHOPIFY.value},
        }

    def order_sync(self, domain_id):
        credential = self.get_credentials(domain_id)
        orders = [
            self.__mapped_customer_shopify_order(order)
            for order in self.__get_orders(credential)
            if order
        ]
        for order in orders:
    ...
    def __export_sync(self, domain_id: int):
        credential = self.get_credentials(domain_id)
        syncs = self.integrations_user_sync_persistence.get_filter_by(
            domain_id=domain_id
        )
        for sync in syncs:
            leads_list = self.lead_persistence.get_leads_domain(
                domain_id=sync.domain_id, status=sync.filter_by_contact_type
            )
            for lead in leads_list:
                self.__create_or_update_shopify_customer(
                    self.lead_persistence.get_lead_data(lead.five_x_five_user_id),
    ...
    def __import_sync(self, user_id: int):
        ...
        customers = [
            ...
            for customer in self.__get_customers(
                credentials.shopify.shop_domain, credentials.shopify.access_token
            )
        ]
    ...
    def __mapped_customer(self, customer) -> ShopifyCustomer:
        ...
        return ShopifyCustomer(
            ...
            email_marketing_consent_opt_in_level=email_marketing_consent.get(
                "opt_in_level"
            ),
