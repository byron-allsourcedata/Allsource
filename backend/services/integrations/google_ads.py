...
class GoogleAdsIntegrationsService:
    ...
    def add_integration(
        ...
        response = self.__handle_request(
            method="POST", url="https://oauth2.googleapis.com/token", json=data
        )
    ...
    def __add_profile_to_list(
        ...
        try:
            ...
            if (
                ...
                and "Too many requests" in message
            ):
    ...
    def create_list(self, list, domain_id, user_id):
        try:
            ...
            channel = self.create_customer_match_user_list(
                client, list.customer_id, list.name
            )
            if not channel:
                return {
            ...
            return {
                "status": IntegrationsStatus.SUCCESS.value,
                "channel": {"list_id": list_id, "list_name": list.name},
            }
        except GoogleAdsException as ex:
            logger.error(
                f"Request with ID '{ex.request_id}' failed with status "
    ...
    def add_users_to_customer_match_user_list(
        ...
        request.operations.extend(
            self.build_offline_user_data_job_operations(client, profiles)
        )
    ...
    def build_offline_user_data_job_operations(
        ...
        for record in raw_records:
            ...
            if "first_name" in record:
                ...
                hashed_street_address = self.normalize_and_hash(
    ...
    def get_user_lists(self, domain_id: int, customer_id: int, user_id: int):
        try:
            ...
            for row in response:
                ...
                user_lists.append(
                    {"list_id": user_list.id, "list_name": user_list.name}
                )
            return {
                "status": IntegrationsStatus.SUCCESS.value,
    ...
    def get_ad_accounts(self, domain_id, user_id):
        try:
            ...
            details = googleads_error.failure.errors[0].message
            error_code = googleads_error.failure.errors[
                0
            ].error_code.authentication_error.name
            logger.error(f"Google Ads API error occurred: {googleads_error}")
