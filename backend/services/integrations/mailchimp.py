...
class MailchimpIntegrationsService:
    ...
    def add_integration(
        ...
        try:
            lists = self.get_list(
                api_key=credentials.mailchimp.api_key, domain_id=domain.id, user_id=user.get('id'),
                server=data_center
            )
            if not lists:
        ...
        integration = self.__save_integation(
            domain_id=domain.id, api_key=credentials.mailchimp.api_key,
    ...
    async def process_data_sync(
        ...
        for enrichment_user in enrichment_users:
            profile = self.__mapped_member_into_list(
                enrichment_user, target_schema, validations, integration_data_sync.data_map
            )
            if profile:
    ...
    def __mapped_member_into_list(
        ...
        business_email, personal_email, phone = self.sync_persistence.get_verified_email_and_phone(user_id)
