...
class BigcommerceIntegrationsService:
    ...
    def connect_integration(
        ...
        return self.bigcommerce_redirect_login(
    ...
    def add_external_apps_install(
        self, new_credentials: IntegrationCredentials, domain_url: str
    ):
        try:
            epi = self.eai_persistence.get_epi_by_filter_one(
    ...
    def add_integration(
        ...
        info = self.__get_store_info(
            store_hash=eai.store_hash, access_token=eai.access_token
        )
        if not info:
            raise HTTPException(
                status_code=409,
    ...
    def oauth_bigcommerce_load(self, signed_payload, signed_payload_jwt):
        try:
            payload = BigcommerceApi.oauth_verify_payload(
                signed_payload, os.getenv("BIGCOMMERCE_CLIENT_SECRET")
            )
            payload_jwt = BigcommerceApi.oauth_verify_payload_jwt(
                signed_payload_jwt,
                os.getenv("BIGCOMMERCE_CLIENT_SECRET"),
                os.getenv("BIGCOMMERCE_CLIENT_ID"),
            )
        except JWTError:
            raise HTTPException(
        ...
        if not user or not owner or not store_hash:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Missing required information in payload",
            )
    ...
    def oauth_bigcommerce_uninstall(self, signed_payload, signed_payload_jwt):
        ...
        if user_integration:
            self.integrations_persistence.delete_integration(
                user_integration.domain_id, user_integration.service_name
            )
            self.domain_persistence.update_pixel_installation(
                user_integration.domain_id, False
            )

        return "The BigCommerce Uninstall Was Successful"

    def add_integration_with_app(
        self, new_credentials: IntegrationCredentials, domain, user: dict
    ):
        credentials = self.get_credentials(domain_id=domain.id)
        info = self.__get_store_info(
            store_hash=new_credentials.bigcommerce.shop_domain,
    ...
    def __set_pixel(self, user_id, domain, shop_domain: str, access_token: str):
        ...
        response_event = self.__handle_request(
            url,
            method="POST",
            access_token=access_token,
            headers=headers,
            json=script_event_data,
        )
    ...
    def __get_orders(self, store_hash: str, access_token: str):
        ...
        while True:
            params = {"min_date_created": date, "limit": limit, "page": page}
            response = self.__handle_request(
                url, method="GET", access_token=access_token, params=params
            )
            ...
            data = response.json()
            paid_orders = [
                order for order in data if order["status_id"] not in {0, 4, 5}
            ]
    ...
    def order_sync(self, domain_id: int):
        credential = self.get_credentials(domain_id)
        orders = [
            self.__mapped_order(order)
            for order in self.__get_orders(
    ...
    def filter_customers_based_on_criteria(
        self,
    ):
        pass
