...
class IntegrationService:
    ...
    def get_user_service_credentials(self, domain, user):
        ...
        return self.integration_persistence.get_integration_by_user(
            domain_id, filters, user.get("id")
        )
    ...
    def get_all_audience_sync(
        ...
        return self.integrations_user_sync_persistence.get_all_audience_sync(
            service_name=service_name,
            integrations_users_sync_id=integrations_users_sync_id,
            user_id=user.get("id"),
        )
    ...
    def get_sync_by_hook_url(self, hook_url):
        return self.integrations_user_sync_persistence.get_data_sync_filter_by(
            hook_url=hook_url
        )
    ...
    def is_integration_limit_reached(self, user: dict, domain: UserDomains):
        ...
        integration_limit, domain_integrations_count = (
            self.integrations_user_sync_persistence.get_limits_integrations(
                user.get("id"), domain.id
            )
        )
        if (
            integration_limit != self.UNLIMITED
            and domain_integrations_count >= integration_limit
        ):
            return True
        return False
    ...
    def switch_sync_toggle(self, domain_id, list_id):
        result = self.integrations_user_sync_persistence.switch_sync_toggle(
            domain_id=domain_id, list_id=list_id
        )
