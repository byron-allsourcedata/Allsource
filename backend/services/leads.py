...
class LeadsService:
                ...
                leads_list.append(
                    {
                        ...
                        "direct_number": (
                            ", ".join(
                                num.strip()
                                for num in lead[6].split(",")
                                if all(
                                    num.strip() != direct.strip()
                                    for direct in lead[4].split(",")
                                )
                            )
                            or None
                        )
                        ...
                        "personal_state": self.convert_state_code_to_name(
                            lead[11], state_dict
                        ),
                        "company_name": lead[12],
                        "company_domain": lead[13],
                        "company_phone": self.format_phone_number(lead[14])
                        if lead[14]
                        else None,
                        "company_sic": lead[15],
                        "company_address": lead[16],
                        "company_city": lead[17],
                        "company_state": self.convert_state_code_to_name(
                            lead[18], state_dict
                        ),
                        ...
                        "professional_state": self.convert_state_code_to_name(
                            lead[43], state_dict
                        ),
    ...
    def download_leads(
        ...
        for five_x_five_user in result_five_x_five_users:
            ...
            page_visits_with_parameters_str = (
                "\n".join(page_visits_info_with_parameters)
                if page_visits_info_with_parameters
                else "None"
            )
            max_spent_time_minutes = max_spent_time // 60
            remaining_seconds = max_spent_time % 60
            time_in_minutes_and_seconds = (
                f"{max_spent_time_minutes} min {remaining_seconds} sec"
            )
            ...
            relevant_data = [
                ...
                self.convert_state_code_to_name(
                    five_x_five_user.personal_state, state_dict
                )
                or "None",
                ...
                f"{five_x_five_user.age_min} - {five_x_five_user.age_max}"
                if five_x_five_user.age_min is not None
                and five_x_five_user.age_max is not None
                else "None",
                ...
                self.convert_state_code_to_name(
                    five_x_five_user.company_state, state_dict
                )
                or "None",
    ...
    def get_leads_for_build_an_audience(
        self,
        regions,
        professions,
        ages,
        ...
        leads_data, count_leads, max_page = (
            self.leads_persistence_service.filter_leads_for_build_audience(
                regions=regions,
        ...
        leads_list = [
            ...
            for id, first_name, last_name, business_email, gender, age_min, age_max, job_title, city, state in leads_data
        ]
        return {
    ...
    def search_location(self, start_letter):
        ...
        for location in location_data:
            results_hash = {"city": location[0].title(), "state": location[1]}
