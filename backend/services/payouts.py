...
class PayoutsService:
    ...
    def process_monthly_payouts(self, payouts: List) -> List[Dict]:
        ...
        for month_year, month_payouts in grouped_by_month.items():
            ...
            rewards_paid = rewards_paid = sum(
                ...
                if payout.status == PayoutsStatus.PAID.value
            )
            ...
            payout_date_formatted = (
                payout_date.strftime("%b %d, %Y") if payout_date else None
            )
    ...
    def process_partners_payouts(
        self, payouts: List, search_query: str, is_master: bool
    ) -> List[Dict]:
        payouts = {payout.parent_id: payout for payout in payouts}.values()
        user_ids = {payout.parent_id for payout in payouts}
        partners = self.partners_persistence.get_partners_by_user_ids(
        ...
        for payout in payouts:
            ...
            if partner:
                ...
                referral_payouts_for_partner = referral_payouts_dict.get(
                    payout.parent_id, []
                )
                ...
                payout_date = max(
                    (
                        referral.paid_at
                        for referral in referral_payouts_for_partner
                        if referral.paid_at is not None
                    ),
                    default=None,
                )
                    ...
                    result.append(
                        {
                            ...
                            "reward_status": self.check_pending_referral_payouts(
                                referral_payouts_for_partner
                            ),
                            "is_payment_active": self.check_payment_active_payouts(
                                referral_payouts_for_partner
                            ),
                        }
                    )
            ...
            processed_payouts.append(
                {
                    ...
                    "join_date": referral_payout.created_at.strftime("%Y-%m-%d")
                    if referral_payout.created_at
                    else None,
    ...
    def is_transfer_successful(self, transfer_response):
        return (
            transfer_response.get("id") is not None
            and not transfer_response.get("reversed", True)
            and transfer_response.get("amount") > 0
        )
    ...
    def pay_out_referrals(self, partner_id):
        ...
        if result:
            ...
            result_transfer = self.stripe_service.create_stripe_transfer(
                total_reward, stripe_account_id
            )
            if self.is_transfer_successful(result_transfer):
                for payout_id in payout_ids:
                    self.referral_payouts_persistence.update_payouts_partner_status(
                        payout_id, PayoutsStatus.PAID.value
                    )
    ...
    def pay_overview_payout_history(self, page, per_page, from_date, to_date):
        history_list, total_count, max_page = (
            self.referral_payouts_persistence.get_overview_payout_history(
