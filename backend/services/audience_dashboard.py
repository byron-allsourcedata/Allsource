...
class DashboardAudienceService:
    ...
    async def get_contacts_for_pixel_contacts_statistics(self, *, user: dict):
        ...
        result_array = [
            {"domain": domain, **stats} for domain, stats in daily_data.items()
        ]
    ...
    def merge_and_sort(self, *, datasets: list[tuple], limit: int):
        ...
        sorted_combined = sorted(
            combined, key=lambda x: x.get("created_at"), reverse=True
        )
        return sorted_combined[:limit]

    def merge_data_with_chain(self, data, type):
        ids = [obj.get("id") for obj in data]
        stmt_results = self.dashboard_persistence.get_chains_data_syncs(
            ids=ids, type=type
        )
        chains = []
        for (
    ...
    def get_events(self, *, user: dict):
        smart_audiences, data_syncs = (
            self.dashboard_persistence.get_last_smart_audiences_and_data_syncs(
                user_id=user.get("id"), limit=self.LIMIT
            )
        )
        sources, lookalikes = (
            self.dashboard_persistence.get_last_sources_and_lookalikes(
                user_id=user.get("id"),
                limit=self.LIMIT,
                smart_audiences=smart_audiences,
            )
        )
    ...
    def get_contacts_for_pixel_contacts_by_domain_id(
        ...
        *,
        ...
        return {"daily_data": dict(daily_data)}
