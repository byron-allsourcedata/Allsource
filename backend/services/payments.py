...
def manage_subscription_schedule(
...
class PaymentsService:
    ...
    def upgrade_subscription(
        ...
        return {
            "status": self.upgrade_subscription_immediate(
                current_subscription, platform_subscription_id, price_id
            )
        }

    def upgrade_subscription_immediate(
        self, current_subscription, platform_subscription_id, price_id
    ):
    ...
    def upgrade_subscription_scheduled(self, current_subscription):
        schedule = stripe.SubscriptionSchedule.retrieve(
            current_subscription.get("schedule")
        )
        stripe.SubscriptionSchedule.release(schedule.id)
    ...
    async def cancel_user_subscription(self, user, reason_unsubscribe):
        ...
        if user.get("source_platform") == SourcePlatformEnum.SHOPIFY.value:
            async with self.integration_service as service:
                ...
                if (
                    subscription_data["status"] == "incomplete"
                    or subscription_data["status"] == "No shopify plan active"
                ):
                    return SubscriptionStatus.INCOMPLETE
        else:
            if user_subscription.platform_subscription_id:
                subscription_data = cancel_subscription_at_period_end(
        ...
        if subscription_data.get("status") == "active":
            ...
            self.plans_service.save_reason_unsubscribe(
                reason_unsubscribe, user.get("id"), cancel_scheduled_at
            )
            ...
            self.plans_service.save_reason_unsubscribe(
                reason_unsubscribe, user.get("id"), time_now
            )
            return SubscriptionStatus.SUCCESS
    ...
    async def upgrade_and_downgrade_user_subscription(
        ...
        if is_downgrade:
            return self.downgrade_subscription(
                current_subscription,
                platform_subscription_id,
    ...
    def charge_user_for_extra_credits(self, quantity: int, users):
        ...
        try:
            purchase_product(
                customer_id,
                self.plans_service.get_additional_credits_price_id(),
                quantity,
                "prospect_credits",
            )
            ...
            return create_stripe_checkout_session(
                ...
                line_items=[
                    {
                        "price": self.plans_service.get_additional_credits_price_id(),
                        "quantity": quantity,
                    }
                ],
                mode="payment",
