...
class AudienceSourceService:
    def __init__(
        ...
        self.audience_sources_matched_persons_persistence = (
            audience_sources_matched_persons_persistence
        )
    ...
    def download_value_calculation(self, source_id: UUID):
        audience_source = self.audience_sources_persistence.get_source_by_id(
            source_id=source_id
        )

        if not audience_source:
            return
        audience_sources_matched_persons, enrichment_persons = (
            self.audience_sources_matched_persons_persistence.get_audience_sources_matched_persons_by_source_id(
                audience_source_id=source_id
            )
        )
        ...
        if audience_source.source_origin == TypeOfSourceOrigin.PIXEL.value:
            ...
            writer.writerow(
                [
                    "",
                    "(reference_date - EVENT_DATE).days",
                    "min(Recency)",
                    "max(Recency)",
                    "1 / (Recency + 1)",
                    "1 / (MinRecency + 1)",
                    ...
                    "duration >= 2 ? 0.5 : duration >= 1 ? 0.25 : 0.0",
                    "RecencyScore + PageViewScore",
                ]
            )

            for matched_person, enrichment_person in zip(
                audience_sources_matched_persons, enrichment_persons
            ):
                relevant_data = [
                    str(matched_person.email) if matched_person.email else "",
                    str(matched_person.recency)
                    if matched_person.recency is not None
                    else "",
                    str(matched_person.recency_min)
                    if matched_person.recency_min is not None
                    else "",
                    str(matched_person.recency_max)
                    if matched_person.recency_max is not None
                    else "",
                    str(matched_person.inverted_recency)
                    if matched_person.inverted_recency is not None
                    else "",
                    str(matched_person.inverted_recency_min)
                    if matched_person.inverted_recency_min is not None
                    else "",
                    ...
                    str(matched_person.recency_score)
                    if matched_person.recency_score is not None
                    else "",
                    str(matched_person.view_score)
                    if matched_person.view_score is not None
                    else "",
                    str(matched_person.value_score)
                    if matched_person.value_score is not None
                    else "",
                ]
                ...
                if audience_source.target_schema == BusinessType.B2C.value:
                    ...
                    for person in audience_sources_matched_persons:
                        relevant_data = [
                            ...
                            str(person.recency_max)
                            if person.recency_max is not None
                            else "",
                            str(person.inverted_recency)
                            if person.inverted_recency is not None
                            else "",
                            str(person.inverted_recency_min)
                            if person.inverted_recency_min is not None
                            else "",
                            ...
                            str(person.amount_max)
                            if hasattr(person, "amount_max")
                            else "",
                            str(person.sum_score)
                            if hasattr(person, "sum_score")
                            else "",
                            str(person.value_score)
                            if person.value_score is not None
                            else "",
                        ]
                        writer.writerow(relevant_data)
                ...
                if audience_source.target_schema == BusinessType.B2B.value:
                    ...
                    for person in audience_sources_matched_persons:
                        relevant_data = [
                            ...
                            str(person.sum_score)
                            if person.sum_score is not None
                            else "",
                            str(person.value_score)
                            if person.value_score is not None
                            else "",
            ...
            if audience_source.source_type == TypeOfCustomer.FAILED_LEADS.value:
                ...
                writer.writerow(
                    [
                        ...
                        "max(Date)",
                        "count(transaction)",
                        "(current_date - LastLeadFailedDate).days",
                        "min(Recency)",
                        "max(Recency)",
                        "1 / (Recency + 1)",
                        "min(InvertedRecency)",
    ...
    def substitution_headings(
        self, source_type: str, headers: List[str]
    ) -> Optional[List[str]]:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a helpful assistant skilled in data mapping.",
                    },
    ...
    async def create_source(
        ...
        creating_data = {
            ...
            "rows": json.dumps([row.dict() for row in payload.rows])
            if payload.rows
            else None,
        }
