...
class CompanyService:
            ...
            company_list.append(
                {
                    "id": company[0],
                    "name": company[1],
                    "phone": self.format_phone_number(company[2])
                    if company[2]
                    else None,
    ...
    def get_employees(
        ...
        employees, count, max_page, states = (
            self.company_persistence_service.filter_employees(
                domain_id=self.domain.id,
                company_id=company_id,
                page=page,
                per_page=per_page,
    ...
    def get_full_information_employee(self, company_id, employee_id):
        employees, states = (
            self.company_persistence_service.get_full_information_employee(
                company_id=company_id,
                employee_id=employee_id,
                domain_id=self.domain.id,
            )
        ...
        employee_data = {
            ...
            "business_email_last_seen": self.get_field_with_status(
                employee[19], employee[27]
            ),
    ...
    def search_location(self, start_letter):
        location_data = self.company_persistence_service.search_location(
            start_letter=start_letter, domain_id=self.domain.id
        )
        results_set = set()
