...
class SubscriptionService:
    ...
    def get_userid_by_customer(self, customer_id):
        result = (
            self.db.query(User, ReferralPayouts.id, ReferralUser.parent_user_id)
            .join(
                ReferralPayouts,
                ReferralPayouts.user_id == User.id,
                isouter=True,
            )
    ...
    def get_billing_history_shopify(self, user_id, page, per_page):
        ...
        billing_history = (
            self.db.query(SubscriptionTransactions)
    ...
    def cancellation_downgrade(self, subscription_id):
        self.db.query(UserSubscriptions).where(
            UserSubscriptions.id == subscription_id
        ).update({"downgrade_at": None, "downgrade_price_id": None})
        self.db.commit()

    def check_duplicate_send(
        self, stripe_request_created_at, platform_subscription_id, price_id
    ):
        subscription_data = (
            self.db.query(SubscriptionTransactions)
            .filter(
                SubscriptionTransactions.price_id == price_id,
                SubscriptionTransactions.platform_subscription_id
                == platform_subscription_id,
            )
            .order_by(SubscriptionTransactions.stripe_request_created_at.desc())
            .first()
        )
        if subscription_data:
            if subscription_data.stripe_request_created_at is not None:
                if isinstance(subscription_data.stripe_request_created_at, str):
                    subscription_datetime = datetime.strptime(
                        subscription_data.stripe_request_created_at,
                        "%Y-%m-%d %H:%M:%S",
                    )
                else:
                ...
                if stripe_request_created_at <= subscription_datetime.replace(
                    tzinfo=None
                ):
                    return True
        return False
    ...
    def is_user_have_subscription(self, user_id):
        return (
            ...
            .filter(SubscriptionPlan.user_id == user_id)
            .limit(1)
            .scalar()
        )
    ...
    def get_user_subscription_with_trial_status(self, user_id):
        ...
        result = self.plans_persistence.get_user_subscription_with_trial_status(
            user_id
        )
        if result:
            (
    ...
    def is_user_has_inactive_subscription_on_basic(self, user_id):
        return self.is_user_has_inactive_subscription_on_basic(user_id=user_id)
        # return self.is_subscription_active(result)
    ...
    def create_payments_transaction(
        self, user_id, stripe_payload, product_description, quantity
    ):
        payment_intent = stripe_payload.get("data", {}).get("object", {})
        transaction_id = payment_intent.get("id")
        users_payments_transactions = (
            self.db.query(UsersUnlockedFiveXFiveUser)
            .filter(UsersUnlockedFiveXFiveUser.transaction_id == transaction_id)
            .first()
        )
        if not users_payments_transactions:
            created_timestamp = stripe_payload.get("created")
            created_at = (
                datetime.fromtimestamp(created_timestamp, timezone.utc).replace(
                    tzinfo=None
                )
    ...
    def create_shopify_subscription_transaction(
        self, subscription_info, user_id, plan: SubscriptionPlan, charge_id
    ):
    ...
    def get_plan_by_title_and_interval(self, plan_type, interval):
        return self.plans_persistence.get_plan_by_title_and_interval(
            plan_type, interval
        )

    def get_plan_by_title_price(self, plan_name, payment_amount):
        return self.plans_persistence.get_plan_by_title_price(
            plan_name, payment_amount
        )

    def create_subscription_transaction(self, user_id, stripe_payload: dict):
        start_date_timestamp = (
            stripe_payload.get("data").get("object").get("current_period_start")
        )
        stripe_request_created_timestamp = stripe_payload.get("created")
        stripe_request_created_at = datetime.fromtimestamp(
            stripe_request_created_timestamp, timezone.utc
        ).replace(tzinfo=None)
        end_date_timestamp = (
            stripe_payload.get("data").get("object").get("current_period_end")
        )
        start_date = datetime.fromtimestamp(
            start_date_timestamp, timezone.utc
        ).replace(tzinfo=None)
        end_date = datetime.fromtimestamp(
            end_date_timestamp, timezone.utc
        ).replace(tzinfo=None)
        created_at = get_utc_aware_date_for_postgres()
        currency = stripe_payload.get("data").get("object").get("currency")
        amount = Decimal(
            stripe_payload.get("data")
            .get("object")
            .get("plan")
            .get("amount_decimal")
        ) / Decimal(100)
        price_id = (
            stripe_payload.get("data").get("object").get("plan").get("id")
        )
        ...
        payment_platform_subscription_id = (
            stripe_payload.get("data").get("object").get("id")
        )
    ...
    def trackAwinConversion(
        self, user: User, price, subscription_type, date, subscription_id
    ):
    ...
    def create_subscription_from_partners(self, user_id):
        plan = self.plans_persistence.get_plan_by_alias(
            PlanAlias.PARTNERS.value
        )
    ...
    def create_subscription_from_free_trial(self, user_id):
        ...
        if user.awin_awc:
            self.trackAwinConversion(
                user,
                0,
                "FREE_TRIAL",
                add_subscription_obj.created_at.strftime("%Y-%m-%d"),
                add_subscription_obj.id,
            )
    ...
    def remove_trial(self, user_id: int):
        ...
        trial_subscription.updated_at = datetime.now(timezone.utc).replace(
            tzinfo=None
        )
        trial_subscription.plan_end = datetime.now(timezone.utc).replace(
            tzinfo=None
        )
        self.db.commit()

    def save_downgrade_price_id(
        self, price_id, subscription: UserSubscriptions
    ):
        subscription.downgrade_price_id = price_id
        subscription.cancel_scheduled_at = None
        subscription.cancellation_reason = None
        subscription.downgrade_at = datetime.now(timezone.utc).replace(
            tzinfo=None
        )
        self.db.commit()
    ...
    def get_subscription_by_price_id(self, price_id):
        return (
            self.db.query(UserSubscriptions)
            .filter(UserSubscriptions.platform_subscription_id == price_id)
            .first()
        )

    def get_additional_credits_price_id(self):
        stripe_price_id = (
            self.db.query(SubscriptionPlan.stripe_price_id)
            .filter(SubscriptionPlan.title == "Additional_prospect_credits")
            .scalar()
        )
    ...
    def update_users_domains(self, user_id, domains_limit):
        ...
        if domains:
            ...
            for i, domain in enumerate(sorted_domains, start=1):
                domain.is_enable = (
                    i <= domains_limit or domains_limit == self.UNLIMITED
                )
        self.db.commit()
    ...
    def update_team_members(self, team_owner_id, members_limit):
        if team_owner_id and members_limit != self.UNLIMITED:
            users = (
                self.db.query(Users)
                .filter(Users.team_owner_id == team_owner_id)
                .all()
            )
            if users:
                sorted_users = sorted(
                    users, key=lambda x: x.added_on or datetime.min
                )
    ...
    def get_plan_by_price(self, lead_credit_price):
        return (
            self.db.query(SubscriptionPlan)
            .filter(SubscriptionPlan.full_price == lead_credit_price)
            .first()
        )

    def process_shopify_subscription(
        ...
        if status == "active":
            ...
            user.stripe_payment_url = (
                None if user.stripe_payment_url else user.stripe_payment_url
            )
    ...
    def process_subscription(
        self, user: Users, stripe_payload, payout_id, referral_parent_id
    ):
        result = {"status": None, "contact_credit_plan_id": None}
        user_id = user.id
        ...
        if stripe_status == "trialing":
            ...
            if interval == "month":
                actual_end_date = actual_start_date + relativedelta(
                    months=+interval_count
                )
            elif interval == "year":
                actual_end_date = actual_start_date + relativedelta(
                    years=+interval_count
                )
        ...
        if status == "active":
            user_subscription = (
                self.db.query(UserSubscriptions)
                .where(
                    UserSubscriptions.platform_subscription_id
                    == platform_subscription_id,
            ...
            if referral_parent_id and not payout_id:
                partner = self.partners_persistence.get_partner_by_user_id(
                    referral_parent_id
                )
                if partner and partner.commission and partner.is_active:
                    discount = stripe_payload.get("discount")
                    amount_decimal = Decimal(
                        stripe_payload.get("plan").get("amount_decimal")
                    ) / Decimal(100)
                    if discount and discount.get("coupon"):
                        ...
                        if amount_off:
                            discount_amount += Decimal(amount_off) / Decimal(
                                100
                            )
                        if percent_off:
                            discount_amount += (
                                amount_decimal * Decimal(percent_off)
                            ) / Decimal(100)
                        final_amount = max(
                            amount_decimal - discount_amount, Decimal(0)
                        )
                    else:
                    ...
                    self.referral_service.create_referral_payouts(
                        reward_amount,
                        user_id,
                        referral_parent_id,
                        "partner",
                    ...
                    if partner.master_id:
                        master_partner = (
                            self.partners_persistence.get_partner_by_user_id(
                                partner.master_id
                            )
                        )
                        if (
                            master_partner
                            and master_partner.commission
                            and master_partner.is_active
                        ):
                            master_reward_amount = round(
                                Decimal(
                                    master_partner.commission
                                    - partner.commission
                                )
                                / Decimal(100)
                                * reward_amount,
            ...
            if user_subscription and user_subscription.status == "active":
                if canceled_at:
                    user_subscription.cancel_scheduled_at = (
                ...
                if start_date > user_subscription.plan_start:
                    ...
                    user_subscription.contact_credit_plan_id = (
            ...
            if user.awin_awc:
                if stripe_status == "trialing":
                    self.trackAwinConversion(
                        ...
                        0,
                        "FREE_TRIAL",
                        user_subscription.plan_start.strftime("%Y-%m-%d"),
