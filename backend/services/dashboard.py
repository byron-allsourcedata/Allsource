...
class DashboardService:
    ...
    def get_revenue(self, from_date, to_date, user):
        ...
        results, lifetime_revenue, investment = (
            self.leads_persistence_service.get_revenue_data(
        ...
        roi = (
            (lifetime_revenue - investment) / investment
            if lifetime_revenue > 0 and investment > 0
            else 0
        )
        ...
        response = {
            ...
            "average_order": {
                "average_order_visitors": round(
                    total_visitors / total_orders_visitors, 3
                )
                if total_orders_visitors > 0
                else 0,
                "average_order_abandoned_cart": round(
                    abandoned_cart / total_orders_abandoned_cart, 3
                )
                if total_orders_abandoned_cart > 0
                else 0,
                "average_order_view_products": round(
                    view_products / total_orders_view_products, 3
                )
                if total_orders_view_products > 0
                else 0,
            },
    ...
    def get_contacts_for_d2c(self, from_date, to_date):
        results = self.leads_persistence_service.get_contact_data_for_d2c(
            domain_id=self.domain.id, from_date=from_date, to_date=to_date
        )
        if results:
            results.sort(key=lambda result: result.start_date)
    ...
    def get_contacts_for_b2b(self, from_date, to_date):
        new_leads_data, returning_visitors_data, page_views_data = (
            self.leads_persistence_service.get_contact_data_for_b2b(
                domain_id=self.domain.id, from_date=from_date, to_date=to_date
            )
        )
        results = []
        all_dates = set(
            [d[0] for d in new_leads_data]
            + [d[0] for d in returning_visitors_data]
            + [d[0] for d in page_views_data]
        )
        ...
        for result in results:
            ...
            total_counts["total_contacts_collected"] += result.get(
                "new_leads", 0
            ) + result.get("returning_visitors", 0)
            total_counts["total_new_leads"] += result.get("new_leads", 0)
            total_counts["total_returning_visitors"] += result.get(
                "returning_visitors", 0
            )
    ...
    def get_contact(self, from_date, to_date, business_type):
        if business_type == "b2b":
            return self.get_contacts_for_b2b(from_date, to_date)
        return self.get_contacts_for_d2c(from_date, to_date)
