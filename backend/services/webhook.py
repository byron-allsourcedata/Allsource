...
class WebhookService:
    ...
    def update_subscription_confirmation(self, payload):
        ...
        if self.subscription_service.check_duplicate_send(
            stripe_request_created_at, platform_subscription_id, price_id
        ):
            return payload

        self.subscription_service.create_subscription_transaction(
            user_id=user_data.id, stripe_payload=payload
        )

        result = self.subscription_service.process_subscription(
            user=user_data,
            stripe_payload=data_object,
            payout_id=payout_id,
            referral_parent_id=referral_parent_id,
        )
        contact_credit_plan_id = result.get("contact_credit_plan_id")
        if result["status"] == "active":
            saved_details_of_payment = save_payment_details_in_stripe(
    ...
    def cancel_subscription_confirmation(self, payload):
            ...
            if stripe_status == "open" or stripe_status == "past_due":
                ...
                save_account_notification = (
                    self.notification_persistence.save_account_notification(
                        user_data.id, account_notification.id
                    )
                )
                stripe_request_created_timestamp = data_object.get("created")
                stripe_request_created_at = datetime.fromtimestamp(
                    stripe_request_created_timestamp
                )
                ...
                message_body["invoice_date"] = datetime.fromtimestamp(
                    data_object.get("amount_due")
                ).strftime("%Y-%m-%d %H:%M:%S")
                message_body["total"] = data_object.get("amount_due")
                message_body["link"] = data_object.get("hosted_invoice_url")

                return {"status": stripe_status, "message_body": message_body}
        ...
        self.subscription_service.process_subscription(
            user=user_data,
            stripe_payload=data_object,
            payout_id=payout_id,
            referral_parent_id=referral_parent_id,
    ...
    def create_payment_confirmation(self, payload):
        ...
        result_status = self.subscription_service.create_payment_from_webhook(
            user_id=user_data.id,
            stripe_payload=payload,
            product_description=product_description,
            quantity=quantity,
    ...
    def shopify_billing_update_webhook(self, payload):
        ...
        charge_id = subscription_info.get("admin_graphql_api_id").split(
            "AppSubscription/"
        )[-1]
        try:
        ...
        with self.integration_service as service:
            ...
            if shopify_charge.billing_on:
                ...
                if delta_days <= 31:
                    ...
                    payment_period = "year"
            plan = self.subscription_service.get_plan_by_title_price(
                plan_name, payment_amount
            )

        self.subscription_service.create_shopify_subscription_transaction(
        ...
        result = self.subscription_service.process_shopify_subscription(
            user=user_data,
            plan=plan,
            subscription_info=subscription_info,
            charge_id=charge_id,
