...
class PartnersService:
    ...
    def partners_by_partner_id(
        self, id, search, start_date, end_date, page, rowsPerPage
    ) -> PartnersObjectResponse:
        offset = page * rowsPerPage
        limit = rowsPerPage
    ...
    def delete_partner(self, id: int, message: str) -> PartnersObjectResponse:
        ...
        try:
            self.send_message_in_email(
                partner.name, message, partner.email, template_id
            )
        ...
        return {
            "message": "Error sending email message. Please try again."
            if not status
            else None
        }
    ...
    async def create_partner(self, new_data: dict) -> PartnersObjectResponse:
        ...
        try:
            token = self.send_referral_in_email(
                new_data.name, new_data.email, new_data.commission
            )
        except Exception as e:
            logger.debug("Error sending mail", e)
            status = False
    ...
    def setUser(self, email: str, user_id: int, status: str, join_date=None):
        self.partners_persistence.update_partner_by_email(
            email=email, user_id=user_id, status=status, join_date=join_date
        )

    async def update_partner(
        self, partner_id: int, new_data: dict
    ) -> PartnersObjectResponse:
        status = True
    ...
    async def update_opportunity_partner(self, partner_id: int, payload: dict):
        ...
        try:
            self.send_message_in_email(
                updated_data.name, message, updated_data.email, template_id
            )
        except Exception as e:
            logger.debug("Error sending mail", e)
            status = False
    ...
    def generate_access_token(self, user: dict, partner_id: int):
        partner = self.partners_persistence.get_partner_by_master(
            parent_user_id=user.get("id"), partner_id=partner_id
        )
        if partner:
            token_info = {
                "id": partner.user_id,
