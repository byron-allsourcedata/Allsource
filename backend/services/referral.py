...
class ReferralService:
    def __init__(
        ...
        referral_persistence_service: ReferralUserPersistence,
        user_persistence: UserPersistence,
        stripe_service: StripeService,
        referral_payouts_persistence: ReferralPayoutsPersistence,
    ):
        self.referral_persistence_discount_code_service = (
    ...
    def get_overview_info(self, user: dict):
        ...
        if user.get("connected_stripe_account_id"):
            account = self.stripe_service.get_stripe_account_info(
                user.get("connected_stripe_account_id")
            )
            if not account:
                account = {}
        ...
        return {
            ...
            "stripe_connected_currently_due": user.get(
                "stripe_connected_currently_due"
            ),
        }
    ...
    def get_referral_discount_code_by_id(
        ...
        discount_code = self.referral_persistence_discount_code_service.get_referral_discount_code_by_id(
            discount_code_id
        )
    ...
    def get_rewards_info(self, year: str, month: int, company_name: str):
        ...
        for month_year, month_payouts in grouped_by_month.items():
            ...
            payout_date = max(
                (payout.created_at for payout in month_payouts), default=None
            )
    ...
    def generate_access_token(self, user: dict, user_account_id: int):
        if self.referral_persistence_service.verify_user_relationship(
            parent_id=user.get("id"), user_id=user_account_id
        ):
            token_info = {
                "id": user_account_id,
