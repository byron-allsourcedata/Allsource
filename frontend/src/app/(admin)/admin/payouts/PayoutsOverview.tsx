...
const faqItems: FAQItem[] = [
	{
		question: "How the referral works?",
		answer:
			"Once a user integrates their Stripe account, they can select a predefined discount code for referrals. A referral code is then generated, which the user can share with their contacts. When a contact signs up using this referral code, the user receives a reward.",
	},
	{
		question: "When will the reward credits be available in my Stripe account?",
		answer:
			"Referral rewards are distributed in the first week of the following month.",
	},
	{
		question: "Who is the official partner?",
		answer:
			"An official partner who refers new users to Maximiz receives higher rewards compared to a regular referral user.",
	},
];
...
const ReferralOverview: React.FC = () => {
	...
	const fetchHistoryList = async (page: number, perPage: number) => {
		try {
			setLoading(true);
			const response = await axiosInstance.get<PayoutsListResponse>(
				"admin-payouts/overview/payout-history",
				{
					params: {
						page: page + 1,
						per_page: perPage,
					...
					>
						...
						>
							...
							>
								...
								<Typography className="table-data">{stripeEmail}</Typography>
							</Box>
							...
							>
								<Typography className="table-heading">Account ID</Typography>
						...
						>
							...
							>
								...
								<Box sx={{ "@media (max-width: 600px)": { display: "none" } }}>
									<CustomTooltip
										title={
											"Our Referral program rewards you for bringing new users to our platform. Share your unique referral link with friends and colleagues, and earn incentives for each successful sign-up."
										}
										linkText="Learn more"
										...
										{payotsList?.length === 0 ? (
											...
											payotsList?.map((payout, index) => (
												...
												>
													...
													>
														{format(
															parse(payout.year_month, "yyyy-MM", new Date()),
															"MMMM, yyyy",
														)}
													</TableCell>
													<TableCell
													...
													<TableCell
														className="table-data"
														sx={{ ...payoutsStyle.tableColumn, pl: 7 }}
													>
