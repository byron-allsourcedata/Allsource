...
const FilterPopup: React.FC<FilterPopupProps> = ({
	...
	const getSelectedJoinDateChip = () => {
		...
		const activeFilter = Object.keys(checkedJoinDateFilters).find(
			(key) =>
				checkedJoinDateFilters[key as keyof typeof checkedJoinDateFilters],
		);
	...
	const handleStatusChange = (options: string[]) => {
		setCheckedFiltersStatuses((prev) => {
			...
			options.forEach((option) => {
				updatedStatuses[option] = !updatedStatuses[option];
			});
					...
					>
						...
						>
							<Box
								sx={{
									...filterStyles.active_filter_dote,
									visibility: isDateFilterJoinDaActive() ? "visible" : "hidden",
								}}
							/>
									...
									<Grid container spacing={0}>
										<Grid item xs={4}>
											<FormControlLabel
												control={
													<Radio
														checked={checkedJoinDateFilters.today}
										...
										<Grid item xs={6}>
											<FormControlLabel
												control={
													<Radio
														checked={checkedJoinDateFilters.last7Days}
														onChange={handleRadioChangeJoinDate}
														name="last7Days"
														size="small"
														sx={{
							...
							>
								<LocalizationProvider dateAdapter={AdapterDayjs}>
									<DatePicker
										...
										onChange={(newValue) =>
											handleJoinDateChange("fromDate")(newValue)
										}
										sx={{ width: "100%" }}
										slots={{
