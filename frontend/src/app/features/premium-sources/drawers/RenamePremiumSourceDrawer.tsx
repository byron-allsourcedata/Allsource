...
import { PremiumSourceUploaded } from "@/components/premium-sources/components/PremiumSourceUploaded";
...
import { useAdminRenamePremiumSource } from "../requests";
...
export const RenamePremiumSourceDrawer: FC<Props> = ({
	...
	const { loading: renameLoading, rename } = useAdminRenamePremiumSource();
		...
		<RenamePremiumSourceDrawerView
			...
			onRename={() => {
				rename(premiumSourceId, nameField.value)
					.then(() => {
						showToast("Premium Source renamed successfully", {
							autoClose: 1500,
						});
						onClose();
					})
					.catch((e: AxiosError) => {
						showErrorToast("Failed to rename premium source");
					});
			}}
			name={nameField}
		/>
		...
		<DrawerContainer>
			...
			<DrawerContent>
				<Column justifyContent="space-between" height="inherit">
					<Column gap="1rem">
						<T>Rename existing Premium Source</T>
