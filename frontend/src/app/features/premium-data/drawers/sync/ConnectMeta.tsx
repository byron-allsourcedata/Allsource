...
export const ConnectMeta: React.FC<ConnectMetaPopupProps> = ({
	open,
	onClose,
	onCloseCreateSync,
	integrationId,
	...
	const [selectedOption, setSelectedOption] = useState<MetaAudience | null>(
		null,
	);
	const [selectedOptionCampaign, setSelectedOptionCampaign] =
		useState<MetaCampaign | null>({
	...
	const [isDropdownOpenCampaign, setIsDropdownOpenCampaign] =
		useState<boolean>(false);
	...
	const [optionAdAccount, setOptionAdAccount] = useState<adAccount | null>(
		null,
	);
	const [metaAudienceList, setMetaAudience] = useState<MetaAudience[]>([]);
	...
	const handleInputChange = (e: any) => {
		...
		const numericValue =
			name === "bidAmount" || name === "dailyBudget" ? Number(value) : value;
	...
	const handleSaveCampaign = () => {
		...
		if (formValues.dailyBudget < 100 || formValues.dailyBudget > 1000000000) {
			showErrorToast(
				"the daily Budget must be more than 100 and less than 1000000000",
			);
			return;
		}
				...
				try {
					...
					if (response.status === 200) {
						...
						const foundItem = response.data?.find(
							(item: any) => item.id === data?.customer_id,
						);
						if (foundItem) {
	...
	const handleSelectOption = (value: MetaAudience | string) => {
		if (value === "createNew") {
	...
	const handleSelectOptionCampaign = (value: MetaAudience | string) => {
	...
	const isKlaviyoList = (value: any): value is MetaAudience => {
		return (
			value !== null &&
			typeof value === "object" &&
			"id" in value &&
	...
	const defaultRows: Row[] = [{ id: 1, type: "Hashed Email", value: "Email" }];
	const [mapping, setMapping] = useState<Row[]>(data?.data_map || defaultRows);
	...
	const handleMapListChange = (
		...
		setMapping(
			mapping.map((row) => (row.id === id ? { ...row, [field]: value } : row)),
		);
	};
	...
	const handleDelete = () => {
		if (selectedRowId !== null) {
			setMapping(mapping.filter((row) => row.id !== selectedRowId));
			handleDeleteClose();
		}
	};
	...
	const createNewCampaign = () =>
		metaCreateCampaign(axiosInstance, {
			setLoading,
			adAccountId: optionAdAccount?.id,
			campaignName: formValues.campaignName,
			bidAmount: formValues.bidAmount,
			dailyBudget: formValues.dailyBudget,
		}).then((campaign: MetaCampaign | undefined) => {
			if (campaign) {
	...
	const handleSaveSync = async () => {
		setLoading(true);
		let list: MetaAudience | null = null;
		try {
											...
											<>
												...
												>
													<TextField
														...
														variant="outlined"
														value={optionAdAccount?.name || null}
														onClick={handleClickAdAccount}
														size="medium"
														disabled={data?.customer_id}
														fullWidth
														label={
															optionAdAccount?.name ? "" : "Select Ad Account"
														}
														InputLabelProps={{
															shrink:
																optionAdAccount?.name ||
																optionAdAccount?.name != ""
																	? false
																	: true,
															sx: {
													...
													>
														...
														{adAccounts?.map((adAccount) => (
															<MenuItem
																key={adAccount.id}
																onClick={() => handleSelectAdAccount(adAccount)}
																sx={{
																	"&:hover": {
												...
												<Box>
													<TextField
														...
														InputLabelProps={{
															shrink: selectedOption?.list_name
																? false
																: isShrunk,
															sx: {
																fontFamily: "var(--font-nunito)",
													...
													>
														...
														{/* Show static options */}
														{metaAudienceList &&
															metaAudienceList.map((klaviyo, option) => (
																<MenuItem
																	key={klaviyo.id}
													...
													<Box>
															...
															InputProps={{
																endAdornment: (
																	<InputAdornment position="end">
																		<IconButton
																			disabled={data?.campaign_name}
																			onClick={handleDropdownToggleCampaign}
																			edge="end"
																		>
														...
														>
															...
															<MenuItem
																disabled={data?.campaign_name}
																onClick={() =>
																	handleSelectOptionCampaign(
																		"createNewAudience",
																	)
																}
																sx={{
																	borderBottom: showCreateFormCampaign
																		? "none"
																		: "1px solid #cdcdcd",
																	"&:hover": {
																		background: "rgba(80, 82, 178, 0.10)",
															...
															{/* Show static options */}
															{metaCampaign?.map((klaviyo) => (
																<MetaCampaignItem
																	key={klaviyo.id}
																	onClick={() =>
																		handleSelectOptionCampaign(klaviyo)
																	}
									...
									{mapping.map((row, index) => (
										<Box key={row.id} sx={{ mb: 2 }}>
											...
											>
												...
												/>
												<Grid item xs="auto" sm={5}>
													<MetaDisabledTextField value={row.value} />
												</Grid>
