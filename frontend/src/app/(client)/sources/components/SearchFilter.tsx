	...
	const fetchDomains = async () => {
		try {
			const response = await axiosInstance.get(
				`/audience-sources/domains?page=${page}&per_page=${perPage}`,
			);
					...
					>
							...
							>
								<Box
									sx={{
										...
										visibility: isSourceFilterActive() ? "visible" : "hidden",
									}}
								/>
								<AllInboxIcon sx={{ fontSize: 20, color: "#5F6368" }} />
								<Typography
									sx={{
								...
								{selectedSource.map((mappedLabel) => {
									const originalLabel = Object.keys(sourceMapping).find(
										(key) => sourceMapping[key] === mappedLabel,
									) as keyof typeof sourceMapping;
									return (
										<CustomChip
											key={mappedLabel}
											label={originalLabel}
											onDelete={() => handleButtonSourceClick(originalLabel)}
										/>
									);
								})}
								<IconButton
								...
								>
									{isSource ? <ExpandLessIcon /> : <ExpandMoreIcon />}
								</IconButton>
							</Box>
						...
						<Box sx={filterStyles.main_filter_form}>
							...
							>
								...
								/>
								<DnsIcon sx={{ fontSize: 20, color: "#5F6368" }} />
						...
						{(selectedSource.length === 0 ||
							...
							<Box sx={filterStyles.main_filter_form}>
								...
								>
									...
									/>
									<WebIcon sx={{ fontSize: 20, color: "#5F6368" }} />
						...
						>
							...
							>
								...
								/>
								<InsertInvitationIcon sx={{ fontSize: 20, color: "#5F6368" }} />
								<Typography
									sx={{
										...filterStyles.filter_name,
									}}
