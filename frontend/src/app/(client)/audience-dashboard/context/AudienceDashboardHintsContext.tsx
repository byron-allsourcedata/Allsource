...
import {
	initialAudienceDashboardHints,
	initialPixelContactsHints,
...
interface HintsContextType {
	changeAudienceDashboardHint: (
		key: AudienceDashboardKey,
		hintKey: HintKey,
		action: HintAction,
	) => void;
	changePixelContactsHint: (
		key: PixelContactsKey,
		hintKey: HintKey,
		action: HintAction,
	) => void;
...
const CompanyHintsContext = createContext<HintsContextType | undefined>(
	undefined,
);

export const AudienceDashboardHintsProvider: React.FC<HintsProviderProps> = ({
	children,
}) => {
	const [audienceDashboardHints, setAudienceDashboardBuilderHints] = useState<
		HintStateMap<AudienceDashboardKey>
	>(initialAudienceDashboardHints);
	const [pixelContactsHints, setPixelContactsHints] = useState<
		HintStateMap<PixelContactsKey>
	>(initialPixelContactsHints);
			...
			value={{
				changeAudienceDashboardHint: (key, hintKey, action) =>
					changeHintState(
						...
						hintKey,
						action,
						setAudienceDashboardBuilderHints,
					),
...
export const useAudienceDashboardHints = () => {
	...
	if (context === undefined) {
		throw new Error(
			"useAudienceDashboardHints must be used within a HintsProvider",
		);
	}
	return context;
};
