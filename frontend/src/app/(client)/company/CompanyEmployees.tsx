...
const CompanyEmployees: React.FC<CompanyEmployeesProps> = ({
	...
	const [firstLockedValue, setFirstLockedValue] = useState<{
		key: string;
		index: number;
	} | null>(null);
	const {
		changeEmployeesTableHint,
		employeesTableHints,
	...
	const getEmployeeById = async (id: number) => {
		...
		try {
			const response = await axiosInstance.get(
				`/company/employee?id=${id}&company_id=${companyId}`,
			);
			if (response.status === 200) {
				...
				setData((prevEmployees) => {
					const index = prevEmployees.findIndex(
						(account) => account.id.value === updateEmployee.id.value,
					);
					if (index !== -1) {
	...
	const fetchEmployeesCompany = async ({
		sortBy,
		sortOrder,
		page,
		rowsPerPage,
		...
		try {
			...
			employees.find((employee: any, index: number) => {
				const hiddenField = Object.entries(employee).find(
	...
	const handleDownload = async () => {
		...
		try {
			...
			const processMultiFilter = (label: string, paramName: string) => {
				...
				if (filter) {
					params.push(
						`${paramName}=${encodeURIComponent(filter.split(", ").join(","))}`,
					);
				}
			};
			processMultiFilter("Department", "department");
			...
			if (selectedFilters.some((filter) => filter.label === "Department")) {
				const regions =
					selectedFilters
						.find((filter) => filter.label === "Department")
						?.value.split(", ") || [];
				if (regions.length > 0) {
	...
	const handleApplyFilters = (filters: FilterParams) => {
		...
		const filterMappings: {
			...
			{
				condition:
					filters.seniority && Object.values(filters.seniority).some(Boolean),
				label: "Seniority",
				value: () => getSelectedValues(filters.seniority!),
			},
			{
				condition:
					filters.jobTitle && Object.values(filters.jobTitle).some(Boolean),
				label: "Job Title",
				value: () => getSelectedValues(filters.jobTitle!),
			},
			{
				condition:
					filters.department && Object.values(filters.department).some(Boolean),
				label: "Department",
	...
	const handleDeleteFilter = (filterToDelete: {
		...
		const filters = JSON.parse(
			sessionStorage.getItem("filters-employee") || "{}",
		);
		const valuesToDelete = filterToDelete.value
			.split(",")
		...
		const newFilters: FilterParams = {
			...
			department: Object.fromEntries(
				Object.keys(filters.department).map((key) => [
					key,
					updatedFilters.some(
						(f) => f.label === "Department" && f.value.includes(key),
					),
				]),
			),
			jobTitle: Object.fromEntries(
				Object.keys(filters.jobTitle).map((key) => [
					key,
					updatedFilters.some(
						(f) => f.label === "Job Title" && f.value.includes(key),
					),
				]),
			),
			seniority: Object.fromEntries(
				Object.keys(filters.seniority).map((key) => [
					key,
					updatedFilters.some(
						(f) => f.label === "Seniority" && f.value.includes(key),
					),
				]),
			),
		};
															...
															>
																...
																{key === firstLockedValue?.key && (
																	<HintCard
																		...
																		positionLeft={-220}
																		positionTop={
																			64.5 * (firstLockedValue.index + 1)
																		}
																		rightSide={true}
																		isOpenBody={
																			employeesTableHints["unlock"].showBody
																		}
																		toggleClick={() => {
																			if (
																				employeesTableHints["download"].showBody
																			) {
																				changeEmployeesTableHint(
																					"download",
																					"showBody",
																			...
																			if (
																				employeesTableHints["overview"].showBody
																			) {
																				changeEmployeesTableHint(
																					"overview",
																					"showBody",
												...
												{data.map((row) => (
													<>
														<TableRow
															...
															sx={{
																height: "68px",
																backgroundColor: selectedRows.has(row.id.value)
																	? "rgba(247, 247, 247, 1)"
																	: "#fff",
																"&:hover": {
														...
														>
															...
															<SmartCell
																...
																tooltipOptions={{
																	content: [
																		capitalizeTableCell(
																			renderField(row.first_name, row.id.value),
																		),
																		capitalizeTableCell(
																			renderField(row.last_name, row.id.value),
																		),
																	]
																		.filter(Boolean)
																		.join(" "),
															...
															<SmartCell
																...
																tooltipOptions={{
																	content: row.linkedin_url?.value
																		? row.linkedin_url.value.replace(
															...
															<SmartCell
																...
																tooltipOptions={{
																	content:
																		[
																			capitalizeTableCell(
																				renderField(row.city, row.id.value),
																			),
																			capitalizeTableCell(
																				renderField(row.state, row.id.value),
																			),
																		]
																			.filter(Boolean)
																			.join(", ") || "--",
							...
							{selectedJobTitle?.split(",").map((part, index) => (
								<Typography
									...
									sx={{
										...
										marginBottom:
											index < selectedJobTitle.split(",").length - 1
												? "4px"
