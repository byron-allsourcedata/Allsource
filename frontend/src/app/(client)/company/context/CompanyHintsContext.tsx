...
import {
	initialCompanyTableHints,
	initialEmployeesTableHints,
...
export const CompanyHintsProvider: React.FC<HintsProviderProps> = ({
	children,
}) => {
	const [companyTableHints, setCompanyTableHints] = useState<
		HintStateMap<CompanyTableKey>
	>(initialCompanyTableHints);
	const [employeesTableHints, setEmployeesTableHints] = useState<
		HintStateMap<EmployeesTableKey>
	>(initialEmployeesTableHints);
...
export const useCompanyHints = () => {
	...
	if (context === undefined) {
		throw new Error(
			"useCompanyHints must be used within a CompanyHintsProvider",
		);
	}
	return context;
};
