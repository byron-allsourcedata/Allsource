    ...
    const fetchData = async ({
        ...
        try {
            ...
            const startEpoch = appliedDates.start
                ? Math.floor(
                      new Date(appliedDates.start.toISOString()).getTime() /
                          1000
                  )
                : null;
            ...
            const endEpoch = appliedDates.end
                ? Math.floor(
                      new Date(appliedDates.end.toISOString()).getTime() / 1000
                  )
                : null;
            ...
            if (
                selectedFilters.some((filter) => filter.label === "From Date")
            ) {
                const fromDate =
                    selectedFilters.find(
                        (filter) => filter.label === "From Date"
                    )?.value || "";
                if (fromDate) {
                    const fromDateUtc = new Date(fromDate);
                    fromDateUtc.setHours(0, 0, 0, 0);
                    const fromDateEpoch = Math.floor(
                        fromDateUtc.getTime() / 1000
                    );
            ...
            if (selectedFilters.some((filter) => filter.label === "To Date")) {
                const toDate =
                    selectedFilters.find((filter) => filter.label === "To Date")
                        ?.value || "";
                if (toDate) {
            ...
            if (error instanceof AxiosError && error.response?.status === 403) {
                if (error.response.data.status === "NEED_BOOK_CALL") {
                    ...
                    error.response.data.status === "PIXEL_INSTALLATION_NEEDED"
                ) {
        ...
        try {
            ...
            if (
                selectedFilters.some(
                    (filter) => filter.label === "Visitor Type"
                )
            ) {
                const status =
                    selectedFilters
            ...
            if (
                selectedFilters.some((filter) => filter.label === "Lead Status")
            ) {
                const funnels =
                    selectedFilters
            ...
            if (selectedFilters.some((filter) => filter.label === "Search")) {
                const searchQuery =
                    selectedFilters.find((filter) => filter.label === "Search")
                        ?.value || "";
                if (searchQuery) {
                    params.push(
            ...
            if (
                selectedFilters.some((filter) => filter.label === "From Time")
            ) {
                const fromTime =
                    selectedFilters.find(
                        (filter) => filter.label === "From Time"
                    )?.value || "";
                if (fromTime) {
            ...
            if (selectedFilters.some((filter) => filter.label === "To Time")) {
                const toTime =
                    selectedFilters.find((filter) => filter.label === "To Time")
                        ?.value || "";
                if (toTime) {
            ...
            if (
                selectedFilters.some((filter) => filter.label === "Time Spent")
            ) {
                const timeSpent =
                    selectedFilters
            ...
            if (
                selectedFilters.some(
                    (filter) => filter.label === "Recurring Visits"
                )
            ) {
                ...
                if (recurringVisits.length > 0) {
                    const formattedRecurringVisits = recurringVisits.map(
            ...
            if (
                selectedFilters.some((filter) => filter.label === "Page Visits")
            ) {
                const pageVisits =
                    selectedFilters
        ...
        const filterMappings: {
            ...
            },
            {
                condition:
                    ...
                    Object.values(filters.checkedFiltersNumberOfEmployees).some(
                        Boolean
                    ),
    ...
    const handleDeleteFilter = (filterToDelete: {
        ...
        switch (filterToDelete.label) {
            ...
            case "Number of Employees":
                Object.keys(filters.checkedFiltersNumberOfEmployees).forEach(
                    (key) => {
        ...
        const newFilters: FilterParams = {
            ...
            to_date: updatedFilters.find((f) => f.label === "To Date")
                ? dayjs(
                      updatedFilters.find((f) => f.label === "To Date")!.value
                  ).unix()
                : null,
            regions: updatedFilters.find((f) => f.label === "Regions")
                ? updatedFilters
                      .find((f) => f.label === "Regions")!
                      .value?.split(", ")
                : [],
            ...
            selectedPageVisit: updatedFilters.find(
                ...
                ? updatedFilters.find((f) => f.label === "Employee Visits")!
                      .value
                : "",
            checkedFiltersNumberOfEmployees: {
                ...Object.keys(filters.checkedFiltersNumberOfEmployees).reduce(
                    (acc, key) => {
                        acc[
                            key as keyof typeof filters.checkedFiltersNumberOfEmployees
                        ] = updatedFilters.some(
                            (f) =>
            ...
            checkedFiltersRevenue: {
                ...Object.keys(filters.checkedFiltersRevenue).reduce(
                    ...
                    {} as Record<
                        keyof typeof filters.checkedFiltersRevenue,
                        boolean
                    >
                ),
                "Below 10k": false,
