...
const CompanyFilterPopup: React.FC<FilterPopupProps> = ({
	open,
	onClose,
	onApply,
	industry,
	...
	const handleDeletePageVisit = (valueToDelete: string) => {
		setSelectedTags((prevTags) => {
			const updatedTags = prevTags.pageVisits.filter(
				(tag) => tag !== valueToDelete,
			);
	...
	// Industry
	const [checkedFiltersIndustries, setCheckedFiltersIndustries] = useState<
	...
	const handleCheckboxChangeRevenue = (event: {
	...
	////Number of Employees
	const [checkedFiltersNumberOfEmployees, setCheckedFiltersNumberOfEmployees] =
		useState({
			"1-10": false,
			"11-25": false,
			"26-50": false,
	...
	const handleCheckboxChangeNumberOfEmployees = (event: {
		target: { name: string; checked: boolean };
	}) => {
		const { name, checked } = event.target;
	...
	// Employee Visits
	const [selectedPageVisit, setSelectedPageVisit] = useState<string | null>(
		null,
	);

	const handleRadioChangePageVisits = (
	...
	const initializeFilters = () => {
		...
		if (savedFilters) {
			...
			setCheckedFiltersNumberOfEmployees(
				savedFilters.checkedFiltersNumberOfEmployees || {
					"1-10": false,
					"11-25": false,
					"26-50": false,
					"51-100": false,
			...
			const isNumberOfEmployeesFilterActive = Object.values(
				savedFilters.checkedFiltersNumberOfEmployees || {},
			...
			if (isNumberOfEmployeesFilterActive) {
				...
				Object.keys(savedFilters.checkedFiltersNumberOfEmployees).forEach(
					(key) => {
			...
			setCheckedFiltersRevenue(
				savedFilters.checkedFiltersRevenue || {
					"1-10": false,
					"11-20": false,
					"21-50": false,
					"51-100": false,
			...
			// Checking active page visit filters
			const isRevenueFilterActive = Object.values(
				savedFilters.checkedFiltersRevenue || {},
			).some((value) => value === true);
			...
			if (isAnyFilterActive) {
				...
				const activeFilter = Object.keys(savedFilters.checkedFilters).find(
					(key) => savedFilters.checkedFilters[key],
				);
	...
	useEffect(() => {
		if (open) {
			if (industry) {
				const initialState = industry.reduce(
					(acc, item) => {
						acc[item] = false;
						return acc;
					},
	...
	const isNumberOfEmployeesFilterActive = () => {
		return Object.values(checkedFiltersNumberOfEmployees).some(
			(value) => value,
		);
	};
	...
	const handleClearFilters = () => {
		...
		setIsNumberOfEmployeeOpen(false), setIsRegionOpen(false);
				...
				>
					...
					<Box sx={filterStyles.main_filter_form}>
						...
						<Collapse in={isVisitedPageOpen}>
							<Box sx={filterStyles.filter_dropdown}>
								<RadioGroup
									row
									value={selectedPageVisit}
									onChange={handleRadioChangePageVisits}
								>
									{[
										{ name: "1", label: "1" },
										{ name: "2", label: "2" },
					...
					<Box sx={filterStyles.main_filter_form}>
						...
						>
							...
							/>
							<Image
								src="/revenue-filter.svg"
								alt="revenue"
								width={18}
								height={18}
							/>
							<Typography sx={filterStyles.filter_name}>Revenue</Typography>
											...
											<FormControlLabel
												control={
													<Checkbox
														checked={
															checkedFiltersRevenue[
																range as keyof typeof checkedFiltersRevenue
															]
														}
					...
					{/* <Box sx={filterStyles.main_filter_form}>
						...
						>
							<Box
								sx={{
									...filterStyles.active_filter_dote,
									visibility: isNumberOfEmployeesFilterActive()
										? "visible"
										: "hidden",
								}}
							/>
							...
							<IconButton
								onClick={() =>
									setIsNumberOfEmployeeOpen(!isNumberOfEmployeeOpen)
								}
								aria-label="toggle-content"
							>
								{isNumberOfEmployeeOpen ? (
									<ExpandLessIcon />
								) : (
									<ExpandMoreIcon />
								)}
					...
					{/* <FormControlLabel
												control={
													<Checkbox
														checked={
															checkedFiltersNumberOfEmployees[
																range as keyof typeof checkedFiltersNumberOfEmployees
															]
														}
												...
												label={
													<Typography
														className="table-data"
														sx={{
															color: checkedFiltersNumberOfEmployees[
																range as keyof typeof checkedFiltersNumberOfEmployees
															]
																? "rgba(56, 152, 252, 1) !important"
																: "rgba(74, 74, 74, 1)",
					...
					<Box sx={filterStyles.main_filter_form}>
						...
						>
							...
							<Image
								...
								height={18}
							/>
							<Typography sx={filterStyles.filter_name}>Industry</Typography>
							<IconButton
								onClick={() => setIsIndustryOpen(!isIndustryOpen)}
								aria-label="toggle-content"
							>
						...
						{Object.keys(checkedFiltersIndustries).some(
