...
const TruncatedText: React.FC<{ text: string; limit: number }> = ({
	...
	return (
		<Box
			...
			sx={{ cursor: shouldTruncate ? "pointer" : "default" }}
		>
			<Typography
				sx={{
					...companyStyles.text,
...
const PopupDetails: React.FC<PopupDetailsProps> = ({
	...
	const handleDownload = async () => {
		try {
			const response = await axiosInstance.get(
				`/company/download-employee/${employeeId}?company_id=${companyId}`,
				{
					responseType: "blob",
				},
			);
	...
	const fetchEmployees = async () => {
		...
		try {
			if (employeeId) {
				const response = await axiosInstance.get(
					`/company/employees/${employeeId}?company_id=${companyId}`,
				);
				if (response.status === 200) {
			...
			>
				...
				>
						...
						<Box sx={{ flex: 1, textAlign: "start" }}>
							...
							>
								<Typography
									variant="body1"
									gutterBottom
									sx={{ ...companyStyles.name, pb: 1 }}
								>
									{[
										capitalizeTableCell(renderField(popupData?.first_name)),
										capitalizeTableCell(renderField(popupData?.last_name)),
									]
										.filter(Boolean)
										.join(" ")}
								</Typography>
								...
								>
									...
									{popupData?.mobile_phone.value ? (
										!popupData?.is_unlocked.value ? (
											...
											<Link
												href={
													`tel:${popupData?.mobile_phone.value?.split(",")[0]}` ||
													"--"
												}
												underline="none"
												sx={{
					...
					<Box sx={companyStyles.box_param}>
						<Typography
							sx={{ ...companyStyles.title_company, alignItems: "center" }}
						>
						...
						<Box sx={{ ...companyStyles.rows_pam, alignItems: "center" }}>
							...
							<Typography sx={{ ...companyStyles.text }}>
								{renderField(
									popupData?.personal_emails_last_seen,
									(time: string) => dayjs(time).format("M/D/YYYY h:mm:ss A"),
								)}
							</Typography>
						</Box>
						...
						<Box sx={{ ...companyStyles.rows_pam, alignItems: "center" }}>
							...
							<Typography sx={{ ...companyStyles.text }}>
								{renderField(
									popupData?.business_emails_last_seen,
									(time: string) => dayjs(time).format("M/D/YYYY h:mm:ss A"),
								)}
							</Typography>
						</Box>
						...
						<Box sx={{ ...companyStyles.rows_pam }}>
							...
							<Typography sx={{ ...companyStyles.text }}>
								{renderField(popupData?.city, (city: string) =>
									[capitalizeTableCell(city)].filter(Boolean).join(", "),
								)}
							</Typography>
						</Box>
						...
						<Box sx={{ ...companyStyles.rows_pam }}>
							<Typography sx={{ ...companyStyles.title_text }}>Zip:</Typography>
							<Typography sx={{ ...companyStyles.text }}>
						...
						>
							...
							<Typography
								sx={{ ...companyStyles.text, color: "rgba(56, 152, 252, 1)" }}
							>
								{!popupData?.is_unlocked.value ? (
									<UnlockButton
										onClick={getStatusCredits}
					...
					<Box sx={companyStyles.box_param}>
						...
						<Box sx={companyStyles.rows_pam}>
							...
							<Typography sx={{ ...companyStyles.text }}>
								<TruncatedText
									text={renderField(popupData?.company_description)}
									limit={100}
								/>
							</Typography>
						</Box>
						...
						<Box sx={companyStyles.rows_pam}>
							...
							<Typography sx={{ ...companyStyles.text }}>
								{renderField(popupData?.company_city, (city: string) =>
									[capitalizeTableCell(city)].filter(Boolean).join(", "),
								)}
							</Typography>
						</Box>
						...
						<Box sx={{ ...companyStyles.rows_pam }}>
							...
							<Typography sx={{ ...companyStyles.text }}>
								{renderField(popupData?.company_last_updated, (time: string) =>
									dayjs(time).format("M/D/YYYY h:mm:ss A"),
								)}
							</Typography>
						</Box>
