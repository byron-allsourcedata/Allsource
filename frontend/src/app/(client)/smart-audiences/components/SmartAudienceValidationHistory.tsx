...
const ValidationsTable = ({
	validations,
}: { validations: ValidationHistoryResponse[] }) => (
	<Box sx={{ width: "100%" }}>
		<Box
			sx={{
				width: "100%",
		...
		>
			...
			>
				<Box sx={{ display: "flex", width: "25%" }}>
					<Typography sx={{ flex: 1 }} className="table-data">
			...
			{validations.map((validationObj, index) => (
				<Box key={index} sx={{ width: "100%" }}>
					{Object.entries(validationObj).map(
						([key, validation]: [string, ValidationHistory], subIndex) => (
							<Box
								key={subIndex}
								sx={{
									width: "100%",
							...
							>
								...
								>
									<Typography sx={{ flex: 1 }} className="black-table-header">
										{setSourceType(setNameValidation(key))}
									</Typography>
									<Box
...
const ValidationsHistoryPopup: React.FC<DetailsPopupProps> = ({
	open,
	onClose,
	id,
	smartAudience,
	...
	const fetchValidationHistory = async () => {
		...
		try {
			const response = await axiosInstance.get(
				`/audience-smarts/${id}/validation-history`,
			);
			if (response.status === 200 && smartAudience) {
				const validationsResponse = response.data;
				const totalCost = validationsResponse.reduce(
					(sum: number, validation: ValidationHistoryResponse) => {
					...
					<Box sx={{ width: "100%", pt: 3, pb: 2, px: 3 }}>
						...
						>
							{smartAudience?.map((el: any, index: number) => {
								...
								const formattedValue = isDateRow
									? dayjs(el.value).format("MM/DD/YYYY")
								...
								return (
									...
									>
										<Typography
											...
											style={{ fontWeight: isDateRow ? 500 : 400 }}
										>
											{el.title}
										</Typography>
										<Typography
											className="black-table-header"
											style={{ fontWeight: isDateRow ? 300 : 400 }}
										>
											{formattedValue}
										</Typography>
									</Box>
