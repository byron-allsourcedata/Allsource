...
const GoogleAdsContactSyncTab: React.FC<GoogleAdsContactSyncTabProps> = ({
	setIsLoading,
	customersInfo,
	...
	const [anchorElGoogle, setAnchorElGoogle] = useState<null | HTMLElement>(
		null,
	);
	const [isDropdownOpenGoogle, setIsDropdownOpenGoogle] =
		useState<boolean>(false);
	const [showCreateFormGoogle, setShowCreateFormGoogle] =
		useState<boolean>(false);
	...
	const [anchorElAdAccountGoogle, setAnchorElAdAccountGoogle] =
		useState<null | HTMLElement>(null);
	const [isDropdownOpenAdAccountGoogle, setIsDropdownOpenAdAccountGoogle] =
		useState(false);
	...
	const handleClickAdAccountGoogle = (
	...
	const isKlaviyoListGoogle = (
		value: ChannelList | string,
	): value is ChannelList => {
		return (
			value !== null &&
			typeof value === "object" &&
			"list_id" in value &&
	...
	const handleNewListChangeGoogle = (
		...
		<>
			<Box sx={{ display: "flex", flexDirection: "column", gap: "24px" }}>
					...
					InputProps={{
						endAdornment: (
							<InputAdornment position="end">
								<IconButton
									onClick={handleDropdownToggleAdAccountGoogle}
									edge="end"
								>
									{isDropdownOpenAdAccountGoogle ? (
										<Image
				...
				<Menu
					...
					open={
						Boolean(anchorElAdAccountGoogle) && isDropdownOpenAdAccountGoogle
					}
					onClose={handleCloseAdAccountGoogle}
					PaperProps={{
						sx: {
							width: anchorElAdAccountGoogle
								? `${anchorElAdAccountGoogle.clientWidth}px`
								: "538px",
							borderRadius: "4px",
				...
				>
					{customersInfo?.map((account: Customers) => (
						<MenuItem
							...
							onClick={() => handleSelectAdAccountGoogle(account)}
							sx={{
								"&:hover": {
			...
			<ClickAwayListener onClickAway={() => {}}>
				<Box>
					...
					<Menu
						...
						PaperProps={{
							sx: {
								width: anchorElGoogle
									? `${anchorElGoogle.clientWidth}px`
									: "538px",
								borderRadius: "4px",
								border: "1px solid #e4e4e4",
							},
					...
					>
						<MenuItem
							...
							sx={{
								borderBottom: showCreateFormGoogle
									? "none"
									: "1px solid #cdcdcd",
								"&:hover": {
									background: "rgba(80, 82, 178, 0.10)",
								...
								>
									...
									>
										<TextField
											...
											InputProps={{
												endAdornment: newListNameGoogle && (
													<InputAdornment position="end">
														<IconButton
															edge="end"
						...
						{googleList &&
							googleList?.map((klaviyo) => (
								<MenuItem
									key={klaviyo.list_id}
									onClick={() => handleSelectOptionGoogle(klaviyo)}
									sx={{
										"&:hover": {
											background: "rgba(80, 82, 178, 0.10)",
										},
