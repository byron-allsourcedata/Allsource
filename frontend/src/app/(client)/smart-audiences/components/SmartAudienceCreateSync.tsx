...
const CreateSyncPopup: React.FC<AudiencePopupProps> = ({
	open,
	onClose,
	updateSmartAudStatus,
	integrationsList: integ = [],
	id,
	...
	const [activeImageService, setActiveImageService] =
		useState<string>("csv-icon.svg");
	...
	useEffect(() => {
		const fetchData = async () => {
			const response = await axiosInstance.get(
				"/integrations/smart-audience-sync/",
				{
					params: {
			...
			if (response.status === 200) {
				setIntegrations([
					{ service_name: "CSV", data_sync: true, id: 0 },
					...response.data,
				]);
			}
		};
	...
	const handleActive = (service: string) => {
		...
		setActiveImageService(
			integrationsImage.filter((item) => item.service_name === service)[0]
				.image,
		);
	};
		...
		try {
			...
			if (activeService === "meta") {
				...
				if (
					optionAdAccountMeta &&
					selectedOptionMeta &&
					optionAdAccountMeta.id &&
					selectedOptionMeta.list_name &&
					selectedOptionMeta.id
				) {
					...
					if (selectedOptionCampaignMeta?.id) {
						...
						if (formValuesMeta?.campaignObjective?.trim()) {
							campaign.campaign_objective = String(
								formValuesMeta.campaignObjective,
							);
						}
			...
			if (activeService === "google_ads") {
				...
				if (
					selectedOptionGoogle?.list_id &&
					selectedOptionGoogle?.list_name &&
			...
			const response = await axiosInstance.post(
				"/data-sync/create-smart-audience-sync",
				requestObj,
				{
					params: {
	...
	const [selectedOptionMailchimp, setSelectedOptionMailchimp] =
	...
	const [selectedOptionMeta, setSelectedOptionMeta] =
		useState<MetaAuidece | null>(null);
	const [selectedOptionCampaignMeta, setSelectedOptionCampaignMeta] =
		useState<MetaCampaign | null>(null);
	const [optionAdAccountMeta, setOptionAdAccountMeta] =
	...
	const [selectedOptionGoogle, setSelectedOptionGoogle] =
		useState<ChannelList | null>();
	const [selectedAccountIdGoogle, setSelectedAccountIdGoogle] =
		useState<string>("");
				...
				>
					...
					>
						...
						{isDownloadAction
							? "with CSV"
							: activeService
								? `with ${formatServiceName(toCamelCase(activeService))}`
								: ""}
					</Typography>
					<Box
						sx={{
									...
									>
										<CardContent sx={{ width: "100%" }}>
											...
											<Typography
												...
												sx={{ marginBottom: 2 }}
											>
												Enter the number of contacts you want to sync. The cost
								...
								>
									...
									>
										...
										{(activeService === "mailchimp" ||
											...
											>
												{(activeService === "mailchimp" &&
													selectedOptionMailchimp?.list_name) ||
													(activeService === "meta" &&
														selectedOptionMeta?.list_name)}
											</Typography>
										)}
