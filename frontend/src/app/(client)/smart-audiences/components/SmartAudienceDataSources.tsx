...
const DataSourceTable = ({
	title,
	dataSources,
}: { title: string; dataSources: DataSource[] }) => (
	<Box sx={{ width: "100%" }}>
		<Box
			sx={{
				width: "100%",
		...
		>
			...
			{dataSources?.map((el: DataSource, index: number) => (
				...
				>
					...
					<Typography sx={{ flex: 1 }} className="black-table-header">
						{setSourceType(el.source_type)}
					</Typography>
					<Typography sx={{ flex: 1 }} className="black-table-header">
						{el.size.toLocaleString("en-US")}
					</Typography>
				</Box>
			))}
		</Box>
	...
	const fetchDataSources = async () => {
		...
		try {
			const response = await axiosInstance.get(
				`/audience-smarts/${id}/data-sources`,
			);
			...
			>
				...
				>
					...
					>
						<DataSourceTable
							title="Included"
							dataSources={dataSourcesInclude}
						/>
						<DataSourceTable
							title="Excluded"
							dataSources={dataSourcesExclude}
						/>
					</Box>
				</Box>
			</Drawer>
