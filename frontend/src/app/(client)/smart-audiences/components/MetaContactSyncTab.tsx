...
const MetaContactSyncTab: React.FC<MetaContactSyncTabProps> = ({
	setIsLoading,
	...
	adAccountsMeta,
	optionAdAccountMeta,
	setOptionAdAccountMeta,
	formValues,
	setFormValues,
}) => {
	const [anchorElMeta, setAnchorElMeta] = useState<null | HTMLElement>(null);
	const [anchorElAdAccountMeta, setAnchorElAdAccountMeta] =
		useState<null | HTMLElement>(null);
	const [isDropdownOpenAdAccountMeta, setIsDropdownOpenAdAccountMeta] =
		useState(false);
	const [metaAudienceList, setMetaAudience] = useState<MetaAuidece[]>([]);
	const [metaCampaign, setMetaCampaign] = useState<MetaCampaign[]>([]);
	const [anchorElCampaignMeta, setAnchorElCampaignMeta] =
		useState<null | HTMLElement>(null);
	const [isDropdownOpenCampaignMeta, setIsDropdownOpenCampaignMeta] =
		useState<boolean>(false);
	const [showCreateFormCampaignMeta, setShowCreateFormCampaignMeta] =
		useState<boolean>(false);
	const [isShrunkCampaignMeta, setIsShrunkCampaignMeta] =
		useState<boolean>(false);
	...
	const [isCheckedCampaign, setIsCheckeCampaign] = useState(false);
	...
	useEffect(() => {
		...
		setIsCheckeCampaign(allFieldsFilled);
	}, [formValues]);
	...
	const handleClickAdAccountMeta = (
		event: React.MouseEvent<HTMLInputElement>,
	) => {
		setIsShrunkMeta(true);
	...
	const createNewCampaign = async () => {
		try {
			...
			const newListResponse = await axiosInstance.post(
				...
				{
					ad_account_id: String(optionAdAccountMeta?.id),
			...
			if (newListResponse.status !== 201) {
				...
				const data = newListResponse.data;
				setInputValueCampaignMeta(data.list_name);
			...
			<>
				<Box sx={{ display: "flex", flexDirection: "column", gap: "24px" }}>
					<TextField
						...
						variant="outlined"
						value={optionAdAccountMeta?.name || null}
						...
						InputLabelProps={{
							shrink:
								optionAdAccountMeta?.name || optionAdAccountMeta?.name != ""
									? false
									: true,
							sx: {
								fontFamily: "Nunito Sans",
					...
					<Menu
						...
						PaperProps={{
							sx: {
								width: anchorElAdAccountMeta
									? `${anchorElAdAccountMeta.clientWidth}px`
									: "538px",
								borderRadius: "4px",
					...
					>
						{adAccountsMeta?.map((adAccount: adAccount) => (
							<MenuItem
								key={adAccount.id}
								onClick={() => handleSelectAdAccountMeta(adAccount)}
								sx={{
									"&:hover": {
				...
				<Box>
					...
					>
						...
						{/* Show static options */}
						{metaAudienceList &&
							metaAudienceList.map((klaviyo) => (
								<MenuItem
									key={klaviyo.id}
									onClick={() => handleSelectOptionMeta(klaviyo)}
									sx={{
										"&:hover": {
											background: "rgba(80, 82, 178, 0.10)",
										},
				...
				{selectedOptionMeta && (
					<Box>
						<TextField
							...
							InputLabelProps={{
								shrink: selectedOptionCampaignMeta
									? false
							...
							InputProps={{
								endAdornment: (
									<InputAdornment position="end">
										<IconButton
											onClick={handleDropdownToggleCampaignMeta}
											edge="end"
										>
						...
						>
							...
							<MenuItem
								onClick={() =>
									handleSelectOptionCampaignMeta("createNewAudience")
								}
								sx={{
									borderBottom: showCreateFormCampaignMeta
										? "none"
										: "1px solid #cdcdcd",
									"&:hover": {
										background: "rgba(80, 82, 178, 0.10)",
									...
									>
										<Box sx={{ textAlign: "right" }}>
											...
											<Typography
												...
												paragraph
											>
												We will not run your campaign. Maximiz will create a
												campaign template in your ad account. We won&apos;t run
												anything without your confirmation.
											</Typography>
											<Button
												variant="contained"
												onClick={handleSaveCampaignMeta}
												disabled={!isCheckedCampaign}
												sx={{
													borderRadius: "4px",
													border: "1px solid rgba(56, 152, 252, 1)",
													boxShadow: "0px 1px 2px 0px rgba(0, 0, 0, 0.25)",
							...
							{/* Show static options */}
							{metaCampaign &&
								metaCampaign.map((klaviyo, option) => (
									<MenuItem
										key={klaviyo.id}
