...
const SourcesList: React.FC = () => {
	...
	const createdSmartAudienceSource: SmartAudienceSource = data
		? JSON.parse(data)
		: null;
	const { hasNotification } = useNotification();
	const [loading, setLoading] = useState(false);

	const [createdData, setCreatedData] = useState<SmartAudienceSource>(
		...
		try {
			if (createdData && createdData.id) {
				const response = await axiosInstance.delete(
					`/audience-smarts/${createdData.id}`,
				);
												...
												sx={{
													...
													background: getStatusStyle(
														progressValidation?.total
															? "Ready"
															: preRenderStatus(
																	createdData.status.charAt(0).toUpperCase() +
																		createdData.status.slice(1),
																),
													).background,
													padding: "3px 8px",
													borderRadius: "2px",
