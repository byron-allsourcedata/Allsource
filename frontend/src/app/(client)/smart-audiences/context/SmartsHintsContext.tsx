...
const SmartsHintsContext = createContext<HintsContextType | undefined>(
	undefined,
);

export const SmartsHintsProvider: React.FC<HintsProviderProps> = ({
	children,
}) => {
	const [smartsBuilderHints, setSmartsBuilderHints] = useState<
		HintStateMap<BuilderKey>
	>(initialSmartsBuilderHints);
	const [smartsTableHints, setSmartsTableHints] = useState<
		HintStateMap<TableKey>
	>(initialSmartsTableHints);
	const [createdSmartsHints, setCreatedSmartHints] = useState<
		HintStateMap<CreatedKey>
	>(initialCreatedSmartHints);
