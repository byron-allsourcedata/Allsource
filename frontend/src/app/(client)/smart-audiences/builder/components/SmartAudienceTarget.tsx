...
const SmartAudiencesTarget: React.FC<SmartAudienceTargetProps> = ({
	...
	const handleCalculateActiveSegments = async (value: number) => {
		...
		const selectedValidations = Object.entries(validationFilters!).flatMap(
			([key, validations]) =>
				validations
					.map((validation: Record<string, any>) =>
		...
		const response = await axiosInstance.post(
			"/audience-smarts/validation-cost-calculate",
			{
		...
		<Box sx={{ mb: 4 }}>
			...
			>
				...
				>
					...
					{smartsBuilderHints["targetType"].show && (
						<HintCard
							card={builderHintCards["targetType"]}
							positionLeft={215}
							isOpenBody={smartsBuilderHints["targetType"].showBody}
							toggleClick={() => toggleDotHintClick("targetType")}
							closeClick={() =>
								changeSmartsBuilderHint("targetType", "showBody", "close")
							}
						/>
					)}
				</Box>
								...
								<FormControl variant="outlined">
									...
									{smartsBuilderHints["selectContacts1"].show && !option && (
										<HintCard
											...
											positionLeft={340}
											isOpenBody={
												smartsBuilderHints["selectContacts1"].showBody
											}
									...
									<FormControl variant="outlined">
										...
										{smartsBuilderHints["selectContacts2"].show && option && (
											<HintCard
												...
												toggleClick={() =>
													toggleDotHintClick("selectContacts2")
												}
												closeClick={() =>
													changeSmartsBuilderHint(
														"selectContacts2",
						...
						<Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
							...
							<Box sx={{ display: "flex", flexDirection: "column" }}>
								...
								>
									...
									{smartsBuilderHints["generateActiveSegment"].show && (
										<HintCard
											card={builderHintCards["generateActiveSegment"]}
											positionLeft={320}
											positionTop={20}
											isOpenBody={
												smartsBuilderHints["generateActiveSegment"].showBody
											}
											toggleClick={() =>
												toggleDotHintClick("generateActiveSegment")
											}
											closeClick={() =>
												changeSmartsBuilderHint(
								...
								<Box sx={{ flex: 1, textAlign: "left" }}>
									...
									typeof validationCost === "number" ? (
										availableCredits >= validationCost ? (
											...
											>
												✗ You need{" "}
												{(validationCost - availableCredits).toLocaleString(
													"en-US",
												)}{" "}
				...
				!isValidateActiveSegments && (
					...
					>
						...
						>
							...
							{smartsBuilderHints["validate"].show && (
								<HintCard
									card={builderHintCards["validate"]}
									positionLeft={-295}
									positionTop={20}
									rightSide={true}
									isOpenBody={smartsBuilderHints["validate"].showBody}
									toggleClick={() => toggleDotHintClick("validate")}
									closeClick={() =>
										changeSmartsBuilderHint("validate", "showBody", "close")
									}
								/>
							)}
						</Box>
