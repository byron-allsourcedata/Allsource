...
export const GradientBarChartView: React.FC<Props> = ({
	title,
	data,
	...
	const everythingElseItem = (percent: number) => {
		return {
	...
	if (sortedData.length > MAX_VISIBLE) {
		...
		processedData = [...visibleTop, everythingElseItem(othersTotal)];
	} else {
	...
	const visibleData = shouldShowEverythingElse
		? [...visibleWithoutOther, everythingElseItem(remainingPercent)]
		: visibleWithoutOther;
