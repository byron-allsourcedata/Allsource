...
import {
	initialCMSHints,
	initialDomainSelectorHints,
	initialPixelInstallationHints,
	initialVerifyPixelIntegrationHints,
	initialManualInstallHints,
...
interface HintsContextType {
	...
	changePixelInstallationHint: (
		key: PixelInstallationKey,
		hintKey: HintKey,
		action: HintAction,
	) => void;
	changeVerifyPixelIntegrationHint: (
		key: VerifyPixelIntegrationKey,
		hintKey: HintKey,
		action: HintAction,
	) => void;
	changeManualInstallHint: (
		key: ManualInstallKey,
		hintKey: HintKey,
		action: HintAction,
	) => void;
...
export const GetStartedHintsProvider: React.FC<HintsProviderProps> = ({
	children,
}) => {
	const [cmsHints, setCmsHints] =
		useState<HintStateMap<CMSKey>>(initialCMSHints);
	const [domainSelectorHints, setDomainSelectorHints] = useState<
		HintStateMap<DomainSelectorKey>
	>(initialDomainSelectorHints);
	const [pixelInstallationHints, setPixelInstallationHints] = useState<
		HintStateMap<PixelInstallationKey>
	>(initialPixelInstallationHints);
	const [verifyPixelIntegrationHints, setVerifyPixelIntegrationHints] =
		useState<HintStateMap<VerifyPixelIntegrationKey>>(
			initialVerifyPixelIntegrationHints,
		);
	const [manualInstallHints, setManualInstallHints] = useState<
		HintStateMap<ManualInstallKey>
	>(initialManualInstallHints);
	...
	return (
		<HintsContext.Provider
			value={{
				...
				resetCMSHints: () => resetHintsState(setCmsHints, initialCMSHints),
				cmsHints,
				...
				resetVerifyPixelIntegrationHints: () =>
					resetHintsState(
						setVerifyPixelIntegrationHints,
						initialVerifyPixelIntegrationHints,
					),
