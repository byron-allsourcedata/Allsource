      ...
      >
        ...
        >
          {audienceSize.map((source) => (
            <Box key={source.id} sx={{ width: { xs: '100%', sm: 'auto' }, position: 'relative' }}>
              {source.id === 'extremely' && (
                <Box
                  sx={{
                    position: 'absolute',
              ...
              <Button
                onClick={() => onSelectSize(source.id, source.min_value, source.max_value)}
                sx={{
                  display: 'flex',
                  flexDirection: 'row',
                  alignItems: 'center',
              ...
              >
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
                  <Typography className="paragraph" sx={{ color: 'rgba(32, 33, 36, 1)' }}>
                    {source.label}
                  </Typography>
                  <Typography className="paragraph" sx={{ fontSize: '12px' }}>
                    {source.text}
                  </Typography>
                </Box>
              </Button>
