...
export function RecommendedBadge({
	...
	return (
		...
		>
			<Typography variant="caption" fontWeight={400} sx={{ fontSize: "12px" }}>
				{text}
			</Typography>
		</Box>
	);
...
export function UnRecommendedBadge({
	text = "No recommended fields",
}: RecommendedBadgeProps) {
	return (
		<Box
			sx={{
...
export function FeatureImportanceTable<T extends FeatureObject>({
	...
	const [selectedKeys, setSelectedKeys] =
		useState<(keyof T)[]>(currentFeatures);
				...
				<Grid container alignItems="center">
					...
					<Grid item xs={2} sm={4}>
						{initialFeatures.length === 0 ? (
							...
							<RecommendedBadge
								text={`${initialFeatures.length} fields recommended`}
							/>
						)}
					</Grid>
					<Grid item xs={4} sm={3}>
