...
export const CalculatedSteps: React.FC<CalculatedStepsProps> = ({
	...
	const initialRecommendedByCategory = useMemo<RecommendedByCategory>(() => {
		const {
			audience_feature_importance_b2c: b2c,
			audience_feature_importance_b2b: b2b,
		} = calculatedResults;
	...
	const [selectedByCategory, setSelectedByCategory] =
		useState<RecommendedByCategory>(initialRecommendedByCategory);
	...
	const isDefaultSelection = useMemo(() => {
		return (
			Object.keys(initialRecommendedByCategory) as Array<
				keyof RecommendedByCategory
			>
		).every((cat) => {
