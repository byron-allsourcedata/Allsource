...
const AccountSetup = () => {
	...
	const fetchEditDomain = async () => {
		try {
			...
			const response = await axiosInstance.put(
				`install-pixel/update-domain`,
				{ new_domain: domainName.replace(/^https?:\/\//, "") },
				{
	...
	const handleSubmit = async () => {
		const newErrors = {
			...
			selectedMethodInstall: selectedMethodInstall
				? ""
				: "Please select method install pixel",
			selectedIntegration: selectedIntegration
				? ""
				: "Please choice integration",
		};
		setErrors(newErrors);
	...
	const range_typeBusiness = [{ label: "B2B" }, { label: "D2C" }];
	const method_installingPixel = [
		{
			label: "Manually",
			src: "install_manually.svg",
			setState: setManuallInstall,
			action: installManually,
		},
		{
			label: "Google Tag Manager",
			src: "install_gtm.svg",
			setState: setGoogletagInstall,
			action: () => {
				setGoogleOpen(true);
			},
		},
		{
			...
			setState: setShopifyInstall,
			action: installCMS,
		},
		{
			label: "WordPress",
			src: "install_cms2.svg",
			setState: setWordpressInstall,
			action: installCMS,
		},
		{
			label: "Bigcommerce",
			src: "bigcommerce-icon.svg",
			setState: setBigcommerceInstall,
			action: installCMS,
		},
	];
	const integrations = [
		...
		{ label: "Slack", src: "Slack-icon.svg", setState: setSlackPopupOpen },
		{
			label: "GoogleAds",
			src: "google-ads.svg",
			setState: setGoogleADSPopupOpen,
		},
		{
			label: "BingAds",
			src: "bingads-icon.svg",
			setState: setBingAdsPopupOpen,
		},
	...
	const handleButtonClick = () => {
		axiosInstance
			.post("/install-pixel/send-pixel-code", { email: emailDeveloper })
			.then((response) => {
	...
	const handleInstallBigCommerce = async () => {
		const response = await axiosInstance.get(
			...
			{ params: { store_hash: bigcommerceHash } },
		);
		if (response.status === 200) {
			showToast("Success");
		...
		>
			...
			<Box sx={{ ...styles.headers, overflow: "hidden" }}>
				...
				<Box sx={{ ...styles.nav, position: "relative" }}>
					{selectedMethodInstall === "" && activeTab != 2 && (
						<Button
					...
					>
						<Tab
							...
							sx={{
								...
								color: activeTab === 0 ? "#F45745" : "#707071",
								"&.Mui-selected": {
									color: "#F45745",
								},
								"@media (max-width: 400px)": {
						...
						<Tab
							...
							sx={{
								...
								padding: 0,
								marginRight: 1.5,
								color: activeTab === 1 ? "#F45745" : "#707071",
								"&.Mui-selected": {
									color: "#F45745",
								},
							}}
						...
						<Tab
							...
							sx={{
								...
								padding: 0,
								marginRight: 1.5,
								color: activeTab === 2 ? "#F45745" : "#707071",
								"&.Mui-selected": {
									color: "#F45745",
								},
							}}
						...
						<Tab
							...
							sx={{
								...
								lineHeight: "normal !important",
								padding: 0,
								color: activeTab === 3 ? "#F45745" : "#707071",
								"&.Mui-selected": {
									color: "#F45745",
								},
							}}
						...
						<Tab
							...
							sx={{
								...
								"&::before": {
									...
									width: "30px",
									height: "30px",
									color: activeTab === 3 ? "#fff" : "rgba(32, 33, 36, 1)",
									alignItems: "center",
									justifyContent: "center",
									backgroundColor:
										activeTab === 3
			...
			<Box sx={{ display: "flex", justifyContent: "center", width: "100%" }}>
				<Box sx={{ ...styles.formContainer, overflow: "hidden" }}>
					...
					>
								...
								<Box sx={styles.employeeButtons}>
									{ranges.map((range, index) => (
										<Button
											...
											onTouchStart={() =>
												handleEmployeeRangeChange(range.label)
											}
									...
									>
										...
										>
											...
											<Typography className="paragraph" sx={subtext}>
												Paste the above pixel in the header of your website. The
												header script starts with &lt;head&gt; and ends with
												&lt;/head&gt;.
											</Typography>
										</Box>
										...
										>
											...
											<Box />
											<Typography className="paragraph" sx={subtext}>
												Once the pixel is pasted in your website, wait for 10-15
												mins and verify your pixel.
											</Typography>
										</Box>
												...
												>
													<InputBase
														...
														onChange={(e: ChangeEvent<HTMLInputElement>) =>
															setEmailDeveloper(e.target.value)
														}
														className="paragraph"
														sx={{
									...
									>
												...
												value={
													isFocused
														? domainName
															...
															: ""
														: domainName
															? `https://${domainName.replace(/^https?:\/\//, "")}`
															: "https://"
												}
												sx={{
													mt: 0,
										...
										<Button
											...
											sx={{
												...styles.submitButton,
												opacity:
													accessToken.trim() !== "" || domainName.trim() !== ""
														? 1
														: 0.6,
												pointerEvents:
													accessToken.trim() !== "" || domainName.trim() !== ""
									...
									>
										...
										<Button
											...
											sx={{
												...
												pointerEvents:
													bigcommerceHash.trim() !== "" ? "auto" : "none",
												mb: 2,
												mt: 0,
												backgroundColor:
													bigcommerceHash.trim() !== ""
														? "rgba(244, 87, 69, 1)"
														: "rgba(244, 87, 69, 0.4)",
												"&.Mui-disabled": {
													backgroundColor: "rgba(244, 87, 69, 0.6)",
												...
												>
													...
													>
														...
														{accounts.length === 0 && (
															...
															>
																No accounts available. Please check your Google
																Tag Manager setup.
															</Typography>
														)}
														...
														{containers.length === 0 && selectedAccount && (
															...
															>
																No containers available for the selected
														...
														>
															<Button
																...
																sx={{
																	...styles.submitButton,
																	opacity: isFormValidFirst() ? 1 : 0.6,
																	pointerEvents: isFormValidFirst()
																		? "auto"
																		: "none",
																	backgroundColor: isFormValidFirst()
																		? "rgba(244, 87, 69, 1)"
									...
									>
										...
										>
											...
											<Box sx={{ display: "flex", alignItems: "center" }}>
												<Typography
													className="first-sub-title"
													sx={{ ...maintext, p: 0, fontWeight: "500" }}
												>
											...
											>
												{method_installingPixel.map((range, index) => (
													<Button
														...
														onTouchStart={() =>
															handleMethodInstall(range.label)
														}
														onMouseDown={() => handleMethodInstall(range.label)}
														sx={{
															...getButtonRolesStyles(
																selectedMethodInstall === range.label,
															),
															gap: "8px",
							...
							<>
								...
								<Box
									sx={{
										...
										"@media (max-width: 450px)": { gridTemplateColumns: "1fr" },
									}}
								>
									{integrations.map((range, index) => (
										<Button
											...
											sx={{
												...getButtonRolesStyles(
													selectedIntegration === range.label,
												),
												gap: "8px",
												justifyContent: "flex-start",
												p: "12px",
						...
						{bookADemoPopupOpen && <BookADemoPopup endSetup={endSetup} />}
					</Box>
				</Box>
			</Box>
		</Box>
