...
const BingAdsDataSync: React.FC<BingAdsDataSyncProps> = ({
	...
	const [newCampaignListName, setNewCampaignListName] = useState<string>(
		data?.name ?? "",
	);
	...
	const [isDropdownCampaignOpen, setIsDropdownCampaignOpen] =
		useState<boolean>(false);
	...
	const [inputCampaignListName, setInputCampaignListName] = useState(
		data?.name ?? "",
	);
	...
	const handleSaveSync = async () => {
		...
		try {
			if (isEdit) {
				...
				if (
					selectedOptionCampaign?.campaign_name &&
					selectedOptionCampaign?.campaign_id
				) {
					...
					>
						{notAdsUser ? (
							...
							>
								...
								<Typography
									variant="body2"
									sx={{ color: "#5052b2", fontWeight: "bold" }}
								>
									<Link
												...
												<Box>
													<TextField
														...
														label={
															inputListName
																? ""
																: "Select or Create audience new list"
														}
														InputLabelProps={{
														...
														{audienceList &&
															audienceList?.map((audience) => (
																<MenuItem
																	...
																	onClick={() => handleSelectOption(audience)}
																	sx={{
																		"&:hover": {
																			background: "rgba(80, 82, 178, 0.10)",
																		},
											...
											<ClickAwayListener onClickAway={() => {}}>
												<Box>
													<TextField
														...
														fullWidth
														label={
															inputCampaignListName
																? ""
																: "Select or Create new campaign list"
														}
														InputLabelProps={{
															shrink: inputCampaignListName
																? false
																: isShrunkCampaign,
															sx: {
																fontFamily: "Nunito Sans",
														...
														InputProps={{
															endAdornment: (
																<InputAdornment position="end">
																	<IconButton
																		onClick={handleDropdownCampaignToggle}
																		edge="end"
																	>
																		{isDropdownCampaignOpen ? (
																			<Image
																				src="/chevron-drop-up.svg"
																				alt="chevron-drop-up"
																				height={24}
													...
													<Menu
														...
														PaperProps={{
															sx: {
																width: anchorElCampaign
																	? `${anchorElCampaign.clientWidth}px`
																	: "538px",
																borderRadius: "4px",
																border: "1px solid #e4e4e4",
															},
													...
													>
														<MenuItem
															disabled={data?.name}
															onClick={() =>
																handleSelectOptionCampaign("createNew")
															}
															sx={{
														...
														{campaignList &&
															campaignList?.map((campaign) => (
																<MenuItem
																	...
																	onClick={() =>
																		handleSelectOptionCampaign(campaign)
																	}
																	sx={{
																		"&:hover": {
																			background: "rgba(80, 82, 178, 0.10)",
...
export default BingAdsDataSync;
