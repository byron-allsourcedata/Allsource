...
const DashboardContactB2B: React.FC<DashboardContactProps> = ({
	values,
	setValues,
	appliedDates,
	typeBusiness,
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const contacts_collected = days.map(
					(day) => daily_data[day].contacts_collected || 0,
				);
				const new_leads = days.map((day) => daily_data[day].new_leads || 0);
				const returning_visitors = days.map(
					(day) => daily_data[day].returning_visitors || 0,
				);
