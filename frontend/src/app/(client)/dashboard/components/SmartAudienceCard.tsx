...
const getUseCaseStyle = (useCase: string) => {
	switch (useCase.trim()) {
		case "Postal":
			return (
				<Image src="/postal.svg" alt="postal icon" width={20} height={20} />
			);
		case "Google":
			return (
				<Image src="/google-ads.svg" alt="google icon" width={20} height={20} />
			);
		case "Meta":
		...
		<Box mb={1}>
			...
			<Box display="flex" flexWrap="wrap" gap={1}>
				{parts.map((entry, i) => {
					...
					const colors =
						type === "include"
							? (colorMap.include[tag as keyof typeof colorMap.include] ??
								colorMap.default)
							: (colorMap.exclude[tag as keyof typeof colorMap.exclude] ??
								colorMap.default);
	...
	return (
		...
		>
			<CardContent sx={{ p: 0, "&:last-child": { paddingBottom: 0 } }}>
				...
				<Typography className="dashboard-card-heading" sx={{ mb: 1 }}>
					{name}
				</Typography>
