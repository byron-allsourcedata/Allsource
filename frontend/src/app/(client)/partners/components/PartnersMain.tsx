...
const TruncatedText: React.FC<{
	...
	return (
		<Box
			...
			sx={{
				cursor:
					shouldTruncate && status !== "Invitation sent"
						? "pointer"
						: "default",
			}}
		>
...
const PartnersMain: React.FC<PartnersProps> = ({
	setLoading,
	loading,
	masterId,
	appliedDates,
	...
	const handleOpenMenu = (event: any, rowData: any) => {
		...
		setFileData({
			...rowData,
			fullName: rowData.partner_name,
			companyName: rowData.sources,
		});
		handleFormOpenPopup();
	};
			...
			const response = await axiosInstance.get(`/partners/partners`, {
				params: {
					...
					end_date: appliedDates.end
						? appliedDates.end.toLocaleDateString("en-CA")
						: null,
					page,
	...
	const updateOrAddPartner = (updatedPartner: PartnerData) => {
		setPartners((prevAccounts) => {
			const index = prevAccounts.findIndex(
				(account) => account.id === updatedPartner.id,
			);
			if (index !== -1) {
	...
	const handleLogin = async (user_account_id: number, status: string) => {
		try {
			...
			console.error("Master Partner login error:", error);
		} finally {
			setLoading(false);
		}
	};

	const Toggle: React.FC<{ isActive: boolean; onToggle: () => void }> = ({
		isActive,
		onToggle,
	}) => {
		return (
							...
							<Box sx={{ display: "flex", flexDirection: "column" }}>
								...
								>
									<Table>
										...
										<TableBody>
											{partners.map((data) => {
												const isActive = partnerStates.find(
													(p: any) => p.id === data.id,
												)?.isActive;
												return (
													<TableRow
													...
													>
														...
														>
															...
															>
																...
																{data.status !== "Invitation sent" && (
																	<IconButton
																		onClick={() =>
																			handleLogin(data.id, data.status)
																		}
														...
														>
															{dayjs(data.reward_payout_date).isValid()
																? dayjs(data.reward_payout_date).format(
																		"MMM D, YYYY",
																	)
																: "--"}
														</TableCell>
														...
														>
															{dayjs(data.last_payment_date).isValid()
																? dayjs(data.last_payment_date).format(
																		"MMM D, YYYY",
																	)
																: "--"}
														</TableCell>
														...
														>
															<Toggle
																...
																onToggle={() => handleStatusChange(data.id)}
															/>
														</TableCell>
