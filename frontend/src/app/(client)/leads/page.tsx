...
const Leads: React.FC = () => {
	...
	const fetchData = async ({
		...
		try {
			...
			if (selectedFilters.some((filter) => filter.label === "Visitor Type")) {
				const status =
					selectedFilters
						.find((filter) => filter.label === "Visitor Type")
						?.value.split(", ") || [];
				if (status.length > 0) {
			...
			if (selectedFilters.some((filter) => filter.label === "Regions")) {
				const regions =
					selectedFilters
						.find((filter) => filter.label === "Regions")
						?.value.split(", ") || [];
				if (regions.length > 0) {
			...
			if (selectedFilters.some((filter) => filter.label === "PageUrl")) {
				const pageUrlTags =
					selectedFilters
						.find((filter) => filter.label === "PageUrl")
						?.value.split(", ") || [];
			...
			if (selectedFilters.some((filter) => filter.label === "Lead Status")) {
				const funnels =
					selectedFilters
						.find((filter) => filter.label === "Lead Status")
						?.value.split(", ") || [];
				if (funnels.length > 0) {
					const formattedFunnels = funnels.map((funnel) =>
						funnel.toLowerCase().replace(/\s+/g, "_"),
					);
			...
			if (selectedFilters.some((filter) => filter.label === "Search")) {
				const searchQuery =
					selectedFilters.find((filter) => filter.label === "Search")?.value ||
					"";
				if (searchQuery) {
			...
			if (selectedFilters.some((filter) => filter.label === "From Time")) {
				const fromTime =
					selectedFilters.find((filter) => filter.label === "From Time")
						?.value || "";
				if (fromTime) {
			...
			if (selectedFilters.some((filter) => filter.label === "To Time")) {
				const toTime =
					selectedFilters.find((filter) => filter.label === "To Time")?.value ||
					"";
				if (toTime) {
			...
			if (selectedFilters.some((filter) => filter.label === "Time Spent")) {
				const timeSpent =
					selectedFilters
						.find((filter) => filter.label === "Time Spent")
						?.value.split(", ") || [];
				if (timeSpent.length > 0) {
					const formattedTimeSpent = timeSpent.map((value) =>
						value.replace(/\s+/g, "_"),
					);
			...
			if (
				selectedFilters.some((filter) => filter.label === "Recurring Visits")
			) {
				const recurringVisits =
					selectedFilters
						.find((filter) => filter.label === "Recurring Visits")
						?.value.split(", ") || [];
				if (recurringVisits.length > 0) {
					const formattedRecurringVisits = recurringVisits.map((value) =>
						value.replace(/\s+/g, "_"),
					);
			...
			if (selectedFilters.some((filter) => filter.label === "Page Visits")) {
				const pageVisits =
					selectedFilters
						.find((filter) => filter.label === "Page Visits")
						?.value.split(", ") || [];
				if (pageVisits.length > 0) {
					const formattedPageVisits = pageVisits.map((value) =>
						value.replace(/\s+/g, "_"),
					);
	...
	const handleApplyFilters = (filters: FilterParams) => {
		...
		const filterMappings: {
			...
			{
				condition: filters.selectedStatus?.length,
				label: "Visitor Type",
				value: () => filters.selectedStatus!.join(", "),
			},
			{
				condition: filters.selectedFunnels?.length,
				label: "Lead Status",
				value: () => filters.selectedFunnels!.join(", "),
			},
			{
			...
			{
				condition: filters.pageUrlTags?.length,
				label: "PageUrl",
				value: () => filters.pageUrlTags!.join(", "),
			},
			{
				condition: filters.recurringVisits?.length,
				label: "Recurring Visits",
				value: () => filters.recurringVisits!.join(", "),
			},
			{
	...
	const handleDeleteFilter = (filterToDelete: {
		...
		switch (filterToDelete.label) {
			...
			case "PageUrl":
				...
				break;
			case "Visitor Type":
				filters.selectedStatus = filters.selectedStatus.filter(
					(status: string) => status !== filterToDelete.value,
				);
				break;
			case "Search":
				filters.searchQuery = "";
		...
		const newFilters: FilterParams = {
			...
			to_date: updatedFilters.find((f) => f.label === "To Date")
				...
				: null,
			selectedStatus: updatedFilters.find((f) => f.label === "Visitor Type")
				? updatedFilters
						.find((f) => f.label === "Visitor Type")!
						.value.split(", ")
				: [],
			regions: updatedFilters.find((f) => f.label === "Regions")
				? updatedFilters.find((f) => f.label === "Regions")!.value.split(", ")
				: [],
			pageUrlTags: updatedFilters.find((f) => f.label === "PageUrl")
				? updatedFilters.find((f) => f.label === "PageUrl")!.value.split(", ")
				: [],
			emails: updatedFilters.find((f) => f.label === "Emails")
				? updatedFilters.find((f) => f.label === "Emails")!.value.split(", ")
				: [],
			selectedFunnels: updatedFilters.find((f) => f.label === "Lead Status")
				? updatedFilters
						.find((f) => f.label === "Lead Status")!
						.value.split(", ")
				: [],
			...
			checkedFiltersPageVisits: {
				page: updatedFilters.some(
					(f) =>
						f.label === "Page Visits" && f.value.split(", ").includes("1 page"),
				),
				two_page: updatedFilters.some(
					(f) =>
						f.label === "Page Visits" &&
						f.value.split(", ").includes("2 pages"),
				),
				three_page: updatedFilters.some(
					(f) =>
						f.label === "Page Visits" &&
						f.value.split(", ").includes("3 pages"),
				),
				more_three: updatedFilters.some(
					(f) =>
						f.label === "Page Visits" &&
						f.value.split(", ").includes("more than 3 pages"),
				),
			},
			checkedFiltersTime: {
				morning: updatedFilters.some(
					(f) => f.label === "Time of Day" && f.value === "morning",
				),
				evening: updatedFilters.some(
					(f) => f.label === "Time of Day" && f.value === "evening",
				),
				afternoon: updatedFilters.some(
					(f) => f.label === "Time of Day" && f.value === "afternoon",
				),
				all_day: updatedFilters.some(
					(f) => f.label === "Time of Day" && f.value === "all_day",
				),
			},
			checkedFiltersTimeSpent: {
				under_10: updatedFilters.some(
					(f) =>
						f.label === "Time Spent" &&
						f.value.split(", ").includes("under 10"),
				),
				over_10: updatedFilters.some(
					(f) =>
						f.label === "Time Spent" &&
						f.value.split(", ").includes("10-30 secs"),
				),
				over_30: updatedFilters.some(
					(f) =>
						f.label === "Time Spent" &&
						f.value.split(", ").includes("30-60 secs"),
				),
				over_60: updatedFilters.some(
					(f) =>
						f.label === "Time Spent" &&
						f.value.split(", ").includes("over 60 secs"),
				),
			},
			recurringVisits: updatedFilters.find(
				(f) => f.label === "Recurring Visits",
			)
				? updatedFilters
						.find((f) => f.label === "Recurring Visits")!
						.value.split(", ")
				: [],
			from_time: updatedFilters.find((f) => f.label === "From Time")
				? updatedFilters.find((f) => f.label === "From Time")!.value
				: null,
			to_time: updatedFilters.find((f) => f.label === "To Time")
				? updatedFilters.find((f) => f.label === "To Time")!.value
				: null,
		};
		...
		try {
			...
			if (selectedFilters.some((filter) => filter.label === "Visitor Type")) {
				...
				if (status.length > 0) {
					...
					params.push(
						`behavior_type=${encodeURIComponent(formattedStatus.join(","))}`,
					);
				}
			}
			...
			if (selectedFilters.some((filter) => filter.label === "Lead Status")) {
				...
				if (funnels.length > 0) {
					...
					params.push(
						`status=${encodeURIComponent(formattedFunnels.join(","))}`,
					);
				}
			}
			...
			if (selectedFilters.some((filter) => filter.label === "Time Spent")) {
				...
				if (timeSpent.length > 0) {
					...
					params.push(
						`average_time_sec=${encodeURIComponent(formattedTimeSpent.join(","))}`,
					);
				}
			}
			...
			if (
				...
				if (recurringVisits.length > 0) {
					...
					params.push(
						`recurring_visits=${encodeURIComponent(formattedRecurringVisits.join(","))}`,
					);
				}
			}
			...
			if (selectedFilters.some((filter) => filter.label === "Page Visits")) {
				...
				if (pageVisits.length > 0) {
					...
					params.push(
						`page_visits=${encodeURIComponent(formattedPageVisits.join(","))}`,
					);
				}
			}
						...
						{status === "PIXEL_INSTALLATION_NEEDED" ? (
							<Box sx={{ mr: 2 }}>
								<FirstTimeScreenCommonVariant2
									...
									InfoNotification={{
										Text: "Contacts page will be available after pixel installation",
									}}
									HelpCard={{
