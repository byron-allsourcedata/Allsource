			...
			value={{
				changeSuppressionsHint: (key, hintKey, action) => {
					if (action === "open") {
						...
						changeHintState("rules", "showBody", "close", setSuppressionsHints);
					}

					changeHintState(key, hintKey, action, setSuppressionsHints);
				},
				resetSuppressionsHints: () =>
					resetHintsState(setSuppressionsHints, initialSuppressionHints),
...
export const useSuppressionsHints = () => {
	...
	if (context === undefined) {
		throw new Error("useSuppressionsHints must be used within a HintsProvider");
	}
	return context;
};
