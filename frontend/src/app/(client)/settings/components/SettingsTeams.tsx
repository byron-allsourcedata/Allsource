...
export const SettingsTeams: React.FC = () => {
	...
	const handleChangeUserRole = async (
		...
		try {
			...
			const response = await axiosInterceptorInstance.post(
				"/settings/teams/change-user-role",
				{ invite_user: email, access_level: role },
			);
	...
	const fetchPendingInvitationData = async () => {
		try {
			...
			const response = await axiosInterceptorInstance.get(
				"/settings/teams/pending-invations",
			);
			const data = response.data;
	...
	const handleInviteUsersPopupOpen = async () => {
		try {
			setIsLoading(true);
			const response = await axiosInterceptorInstance.get(
				"/settings/teams/check-team-invitations-limit",
			);
			if (response.status === 200) {
			...
			if (axios.isAxiosError(error)) {
				if (error.response && error.response.status === 403) {
					showErrorToast(
						"Access denied: You do not have permission to send this invitation.",
					);
				} else {
				}
			}
	...
	const handleRevokeInvitation = async (email: string) => {
		try {
			setIsLoading(true);
			const response = await axiosInterceptorInstance.put("/settings/teams", {
				pending_invitation_revoke: email,
			});
			...
			if (axios.isAxiosError(error)) {
				if (error.response && error.response.status === 403) {
					showErrorToast(
						"Access denied: You do not have permission to revoke this invitation.",
					);
				} else {
				}
			}
		...
		<Box sx={{ pr: "16px" }}>
				...
				>
					<Box sx={{ display: "flex", alignItems: "center", gap: "8px" }}>
						...
						<CustomTooltip
							...
							linkText="Learn more"
							linkUrl="https://allsourceio.zohodesk.com/portal/en/kb/articles/team-members"
						/>
					</Box>
					<Box
						sx={{
					...
					>
						<Button onClick={handleInviteUsersPopupOpen}>
							<Image
								src="/add-square.svg"
								alt="add-square"
								height={24}
					...
					<InviteUsersPopup
						open={inviteUsersPopupOpen}
						onClose={handleInviteUsersPopupClose}
						onSend={handleSend}
					/>
				</Box>
						...
						<TableHead>
							<TableRow>
								...
								>
									User
									<IconButton
										sx={{ padding: 0, background: "none", marginLeft: "90px" }}
									>
										<Image
											src={
								...
								teamMembers.map((member) => (
									...
									>
										...
										<TableCell
											className="table-data"
											sx={teamsStyles.tableBodyColumn}
											onClick={(e) => handleDropdownClick(e, member.email)}
										>
											<FormControl
												variant="outlined"
												...
												<Select
													...
													onChange={
														member.access_level === "owner"
															? undefined
															: (e) => handleSelectionChange(e, member.email)
													}
													open={
						...
						>
							<Box display="flex" justifyContent="flex-end" mt={2}>
								<Button
									...
									onClick={handleRevokePopupClose}
									sx={{
										borderRadius: "4px",
			...
			>
				...
				>
					...
					>
						<Image
							src="/remove-team-member-icon.svg"
							alt="delete-card-icon"
							width={403}
							height={403}
						/>
