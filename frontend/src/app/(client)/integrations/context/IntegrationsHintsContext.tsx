...
const IntegrationHintsContext = createContext<
	IntegrationHintsContextType | undefined
>(undefined);

export const IntegrationHintsProvider: React.FC<{ children: ReactNode }> = ({
	children,
}) => {
	const [hints, setHints] = useState<HintStateMap<IntegrationKey>>(
		initialIntegrationHints,
	);
	return (
...
export const useIntegrationHints = () => {
	...
	if (!ctx)
		throw new Error(
			"useIntegrationHints must be used inside IntegrationHintsProvider",
		);
	return ctx;
};
