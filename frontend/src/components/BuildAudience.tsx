...
const BuildAudience: React.FC<BuildAudienceProps> = ({
	open,
	onClose,
	onDataFetch,
}) => {
	...
	const [isNotInExistingListsOpen, setIsNotInExistingListsOpen] =
	...
	const handleApplyFilters = async () => {
		try {
			...
			if (netWorths.length > 0)
				queryParams.append("net_worths", netWorths.join(","));
			if (interestList.length > 0)
				queryParams.append("interest_list", interestList.join(","));
			if (notInExistingLists.length > 0)
				queryParams.append(
					"not_in_existing_lists",
					notInExistingLists.join(","),
				);
			queryParams.append("page", String(1));
			queryParams.append("per_page", String(15));

			const response = await axiosInstance.get(
				`/audience/leads?${queryParams.toString()}`,
			);

			const { leads_list, count_leads, max_page } = response.data;

			if (
				Array.isArray(leads_list) &&
				typeof count_leads === "number" &&
				typeof max_page === "number"
			) {
		...
		>
			...
			>
				...
				>
					...
					<Box sx={{ display: "flex", flexWrap: "wrap", gap: "8px", mb: 2 }}>
						{notInExistingLists.map((tag, index) => (
							<Chip
								...
								label={tag}
								onDelete={() =>
									removeTag(tag, setNotInExistingLists, notInExistingLists)
								}
							/>
						))}
					</Box>
					<IconButton
						onClick={() =>
							setIsNotInExistingListsOpen(!isNotInExistingListsOpen)
						}
						aria-label="toggle-content"
					>
