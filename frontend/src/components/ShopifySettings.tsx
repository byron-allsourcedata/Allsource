...
const ShopifySettings = ({
	...
	const handleShopDomainChange = (
		event: React.ChangeEvent<HTMLInputElement>,
	) => {
		const value = event.target.value;
		setShopDomain(value);
		setShopDomainError(
			value.replace("https://", "") != sessionStorage.getItem("current_domain")
				? "Shop domain not matched"
				: value == ""
					...
					>
						<TabContext value={value}>
							<Box sx={{ pb: 4 }}>
								<TabList
									centered
									aria-label="Connect to Shopify Tabs"
									TabIndicatorProps={{
								...
								>
									<Tab
										...
										value="1"
										sx={{
											...shopifySettingsStyle.tabHeading,
											cursor: "pointer",
										}}
									/>
								</TabList>
							...
							<TabPanel value="1" sx={{ p: 0 }}>
								...
								>
									...
									>
										...
										<Tooltip
											title="Enter the Access Token provided by Shopify"
											placement="right"
										>
											<Image
