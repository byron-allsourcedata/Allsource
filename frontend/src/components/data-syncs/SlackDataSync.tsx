...
const SlackDataSync: React.FC<ConnectSlackPopupProps> = ({
	...
	const getChannelList = async () => {
		try {
			...
			const foundItem = response.data?.channel.find(
				(item: any) => item.name === data?.channel.name,
			);
			if (foundItem) {
	...
	const defaultRows: Row[] = [
		...
		{ id: 4, type: "Company", value: "Company" },
		{
			...
			type: "Detail",
			value:
				"Detail (They show company detail Number of Employee| Revenue| Industry)",
		},
